...
func openAIApiTestDB(t *testing.T) *gorm.DB {
	...
	for authorityID, permissions := range serviceCommon.DefaultRolePermissionMatrix() {
...
func aiAPIEngine(t *testing.T, service *orderfoodService.AIService) *gin.Engine {
	t.Helper()
	previousService := aiService
	aiService = service
	t.Cleanup(func() {
		aiService = previousService
	})
...
func TestAIApiKeyIsWriteOnlyAndPromptBodiesArePermissioned(t *testing.T) {
	...
	createBody := `{
		...
		"apiKey":"sk-api-test-secret-1234",
		"timeoutMs":30000,
		"retryCount":1,
		"enabled":true
	}`
	...
	if strings.Contains(recorder.Body.String(), "sk-api-test-secret-1234") ||
		strings.Contains(recorder.Body.String(), "apiKey") {
		t.Fatalf("API response leaked API key: %s", recorder.Body.String())
	}
	var provider struct {
		ID string `json:"id"`
	}
	...
	if detailEnvelope.Code != 0 ||
		strings.Contains(string(detailEnvelope.Data), "sk-api-test-secret-1234") ||
		strings.Contains(string(detailEnvelope.Data), "apiKey") {
	...
	operatorHeaders := map[string]string{
		"X-Test-Authority": strconv.FormatUint(
			uint64(commonModel.AuthorityOrderFoodOperator),
			10,
		),
	...
	if err := db.Model(&commonModel.AdminAccessAudit{}).Count(&accessCount).Error; err != nil {
