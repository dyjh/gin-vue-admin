...
type AIProviderListQuery struct {
	...
	Type      orderfood.AIProviderType `form:"type" json:"type" binding:"omitempty,oneof=bailian deepseek openai"`      // 类型
...
type AIProviderCreateInput struct {
	...
	Type          orderfood.AIProviderType `json:"type" binding:"required,oneof=bailian deepseek openai"`     // 类型
	BaseURL       string                   `json:"baseUrl" binding:"required,url,max=300" checksql:"false"`   // 基础地址
	CredentialRef string                   `json:"credentialRef" binding:"required,max=300" checksql:"false"` // 供应商凭证环境变量引用，格式为 env://变量名
...
type AIProviderUpdateInput struct {
	Name            string  `json:"name" binding:"required,max=60"`                             // 名称
	BaseURL         string  `json:"baseUrl" binding:"required,url,max=300" checksql:"false"`    // 基础地址
	CredentialRef   *string `json:"credentialRef" binding:"omitempty,max=300" checksql:"false"` // 供应商凭证环境变量引用，格式为 env://变量名
	TimeoutMS       int     `json:"timeoutMs" binding:"required,min=1000,max=120000"`           // 超时时间（毫秒）
	Remark          *string `json:"remark" binding:"omitempty,max=300"`                         // 备注
	ExpectedVersion int64   `json:"expectedVersion" binding:"required,min=1"`                   // 预期版本
}
...
type AIModelListQuery struct {
	...
	Capability orderfood.AIModelCapability `form:"capability" json:"capability" binding:"omitempty,oneof=text vision image_generation"` // 能力
	Enabled    *bool                       `form:"enabled" json:"enabled" binding:"omitempty"`                                          // 是否启用
	SortBy     string                      `form:"sortBy" json:"sortBy" binding:"omitempty,oneof=createdAt updatedAt name"`             // 排序字段
	SortOrder  string                      `form:"sortOrder" json:"sortOrder" binding:"omitempty,oneof=asc desc"`                       // 排序方向
}
...
type AIModelCreateInput struct {
	...
	Capabilities                   []orderfood.AIModelCapability `json:"capabilities" binding:"required,min=1,unique,dive,oneof=text vision image_generation"` // 能力列表
...
type AIPromptConfigInput struct {
	Mode               orderfood.AIPromptMode `json:"mode" binding:"required,oneof=default custom"`                      // 模式，default使用当前平台默认提示词，custom使用自定义提示词
