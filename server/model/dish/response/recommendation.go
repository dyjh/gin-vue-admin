...
type DiscoverableDishSummary struct {
	...
	Category               commonResponse.CatalogReference   `json:"category"`               // 分类
	Tags                   []commonResponse.CatalogReference `json:"tags"`                   // 标签列表
	Author                 commonResponse.UserReference      `json:"author"`                 // 作者
...
type DiscoverableDishDetail struct {
	...
	Ingredients             []contentResponse.DishIngredient         `json:"ingredients"`       // 食材列表
	Steps                   []contentResponse.DishStep               `json:"steps"`             // 步骤列表
	MediaReviewStatus       string                                   `json:"mediaReviewStatus"` // 图片审核状态
	Recommendation          *RecommendationSummary                   `json:"recommendation"`    // 推荐摘要
	ModerationSummary       *contentResponse.ModerationRecordSummary `json:"moderationSummary"` // 图片审核摘要
...
type RecommendationDetail struct {
	...
	CreatedBy               commonResponse.AdministratorSummary `json:"createdBy"`               // 创建管理员
