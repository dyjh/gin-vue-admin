...
type AIProviderListQuery struct {
	...
	Type      model.AIProviderType `form:"type" json:"type" binding:"omitempty,oneof=bailian deepseek openai"`      // 类型
...
type AIProviderCreateInput struct {
	...
	Type      model.AIProviderType `json:"type" binding:"required,oneof=bailian deepseek openai"`   // 类型
...
type AIModelListQuery struct {
	...
	Capability model.AIModelCapability `form:"capability" json:"capability" binding:"omitempty,oneof=text vision image_generation"` // 能力
	Enabled    *bool                   `form:"enabled" json:"enabled" binding:"omitempty"`                                          // 是否启用
	SortBy     string                  `form:"sortBy" json:"sortBy" binding:"omitempty,oneof=createdAt updatedAt name"`             // 排序字段
	SortOrder  string                  `form:"sortOrder" json:"sortOrder" binding:"omitempty,oneof=asc desc"`                       // 排序方向
}
...
type AIModelCreateInput struct {
	...
	Capabilities                   []model.AIModelCapability `json:"capabilities" binding:"required,min=1,unique,dive,oneof=text vision image_generation"` // 能力列表
...
type AIPromptConfigInput struct {
	Mode               model.AIPromptMode `json:"mode" binding:"required,oneof=default custom"`                      // 模式，default使用当前平台默认提示词，custom使用自定义提示词
