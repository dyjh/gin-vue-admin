...
type DishIngredient struct {
	...
	Unit      *commonResponse.CatalogReference `json:"unit"`      // 单位
...
type UserDishAdminSummary struct {
	...
	Name                          string                            `json:"name"`                          // 名称
	Owner                         commonResponse.UserReference      `json:"owner"`                         // 所有者
...
type UserDishSourceOverview struct {
	...
	OriginalAuthor      *commonResponse.UserReference `json:"originalAuthor"`      // 原始作者
...
type ModerationRecordSummary struct {
	...
	User              *commonResponse.UserReference `json:"user"`              // 上传用户摘要
...
type UserDishAdminDetail struct {
	...
	DeletedBy             *commonResponse.AdministratorSummary `json:"deletedBy"`             // 删除人
