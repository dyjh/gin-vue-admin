...
func NewDashboardService(db *gorm.DB, permission *PermissionService) *DashboardService {
	if permission == nil {
...
func (service *DashboardService) GetDashboard(
	ctx context.Context,
	query orderfoodRequest.DashboardQuery,
	actor orderfoodRequest.AdminActor,
) (orderfoodResponse.DashboardData, error) {
	query.ApplyDefaults()
	if service == nil || service.Permission == nil {
		return orderfoodResponse.DashboardData{}, appErrors.AdminInternal.DefaultMsg()
	}
	if err := service.Permission.Require(ctx, actor.AuthorityID, PermissionDashboardRead); err != nil {
		return orderfoodResponse.DashboardData{}, err
	}
	db := service.database()
	if db == nil {
		return orderfoodResponse.DashboardData{}, appErrors.AdminInternal.DefaultMsg()
	...
	return orderfoodResponse.DashboardData{
...
func (service *DashboardService) dashboardMetrics(
...
) ([]orderfoodResponse.DashboardMetric, orderfoodResponse.PlatformCapabilitySnapshot, error) {
	newUsers, err := dashboardCount(
		ctx, db.Model(&orderfoodModel.MiniAppUser{}).
			Where("registered_at >= ? AND registered_at <= ?", start, end),
		"count new users",
	)
	if err != nil {
		return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, err
	}
	activeUsers, err := dashboardCount(
		ctx, db.Model(&orderfoodModel.FrontUserActivityDay{}).
			Distinct("user_id").
	...
	usableDishes, err := dashboardCount(
		ctx, db.Model(&orderfoodModel.UserDish{}).
			Where("status = ?", orderfoodModel.DishStatusUsable),
		"count usable dishes",
	)
	if err != nil {
		return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, err
	...
	discoverableDishes, err := dashboardCount(
		ctx, db.Model(&orderfoodModel.UserDish{}).
			Where("status = ? AND discoverable = ? AND source_locked = ?",
				orderfoodModel.DishStatusUsable, true, false),
	...
	recommendedDishes, err := dashboardCount(
		ctx, db.Model(&orderfoodModel.PlatformRecommendation{}).
	...
	rangedMetrics := make([]orderfoodResponse.DashboardMetric, 0, len(rangedSpecs))
	for _, spec := range rangedSpecs {
		...
		if countErr != nil {
			return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, countErr
		}
	...
	pointsEarned, err := dashboardPointSum(ctx, db, start, end, orderfoodModel.PointEarned)
	if err != nil {
		return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, err
	}
	pointsSpent, err := dashboardPointSum(ctx, db, start, end, orderfoodModel.PointSpent)
	if err != nil {
		return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, err
	}
	pointsRefunded, err := dashboardPointSum(ctx, db, start, end, orderfoodModel.PointRefund)
	if err != nil {
		return nil, orderfoodResponse.PlatformCapabilitySnapshot{}, err
	}
	...
	metrics := []orderfoodResponse.DashboardMetric{
		...
		dashboardMetric("usable_dishes", "可用菜品数", usableDishes, "道", "OrderFoodUserDishes", map[string]interface{}{"status": orderfoodModel.DishStatusUsable}),
	...
	metrics = append(metrics,
		dashboardMetric("points_earned", "获得积分", pointsEarned, "积分", "OrderFoodPointEntries", map[string]interface{}{"type": orderfoodModel.PointEarned, "range": rangeCode}),
		dashboardMetric("points_spent", "消耗积分", pointsSpent, "积分", "OrderFoodPointEntries", map[string]interface{}{"type": orderfoodModel.PointSpent, "range": rangeCode}),
		dashboardMetric("points_refunded", "退款积分", pointsRefunded, "积分", "OrderFoodPointEntries", map[string]interface{}{"type": orderfoodModel.PointRefund, "range": rangeCode}),
	)
...
func (service *DashboardService) dashboardAlerts(
...
) ([]orderfoodResponse.DashboardAlert, error) {
	...
	specs := []alertSpec{
		{
			...
			model: &orderfoodModel.GovernanceJob{},
		...
		},
		{
			...
			model:       &orderfoodModel.ImageModerationRecord{},
		...
		},
		{
			...
			model:       &orderfoodModel.SubscribeMessageLog{},
			where:       "created_at >= ? AND created_at <= ? AND status = ?",
			args:        []interface{}{start, end, orderfoodModel.SubscribeLogFailed},
			targetPage:  "OrderFoodSubscribeLogs",
			targetQuery: map[string]interface{}{"status": orderfoodModel.SubscribeLogFailed, "range": rangeCode},
		},
		{
	...
	alerts := make([]orderfoodResponse.DashboardAlert, 0, len(specs)+2)
	for _, spec := range specs {
		...
		if count > 0 {
			alerts = append(alerts, orderfoodResponse.DashboardAlert{
...
func dashboardPointSum(
	...
	entryType orderfoodModel.PointEntryType,
) (int64, error) {
	var value sql.NullInt64
	if err := db.WithContext(ctx).Model(&orderfoodModel.FrontPointEntry{}).
...
func dashboardAIMetrics(
...
) ([]orderfoodResponse.DashboardMetric, error) {
	var statusRows []struct {
		ExecutionStatus string // 执行状态
		Count           int64  // 调用数量
	}
	if err := db.WithContext(ctx).Model(&orderfoodModel.FrontFeatureUsage{}).
	...
	return []orderfoodResponse.DashboardMetric{
...
func dashboardCapabilityState(
	ctx context.Context,
	db *gorm.DB,
) (orderfoodResponse.PlatformCapabilitySnapshot, error) {
	...
	var policy orderfoodModel.PlatformCapabilityPolicy
	...
	state := orderfoodResponse.PlatformCapabilitySnapshot{
...
) orderfoodResponse.DashboardMetric {
	var target *string
