...
func TestPersistCapabilityConfigUsesOptimisticVersion(t *testing.T) {
	...
	if err := db.AutoMigrate(&aiModel.AICapabilityConfig{}); err != nil {
		t.Fatalf("migrate capability config: %v", err)
	}
	now := time.Now().UTC()
	current := aiModel.AICapabilityConfig{
		CapabilityCode:              aiModel.AICapabilityCheckinImageAnalyze,
		Version:                     1,
	...
	var saved aiModel.AICapabilityConfig
...
func TestHTTPAIConnectorCredentialUsesStoredAPIKey(t *testing.T) {
	...
	value, err := connector.credential(
		context.Background(),
		aiModel.AIProvider{APIKey: " test-api-key "},
	)
	if err != nil {
...
func aiTestActor() aiRequest.AIAdminActor {
	nickname := "AI 管理员"
...
func seedAllAICapabilitiesReady(t *testing.T, db *gorm.DB, now time.Time) {
	...
	provider := aiModel.AIProvider{
		...
		Type: aiModel.AIProviderOpenAI, BaseURL: "https://example.test/v1",
...
func TestAIPlatformPolicySaveAppliesImmediatelyAndEmergencyRemainsIndependent(t *testing.T) {
	...
	users := []userModel.MiniAppUser{
		{ID: "normal-a", OpenIDHash: strings.Repeat("a", 64), OpenIDEncrypted: "enc-a", Nickname: "A", Status: userModel.UserStatusNormal, Version: 1, RegisteredAt: now},
		{ID: "normal-b", OpenIDHash: strings.Repeat("b", 64), OpenIDEncrypted: "enc-b", Nickname: "B", Status: userModel.UserStatusNormal, Version: 1, RegisteredAt: now},
		{ID: "blocked", OpenIDHash: strings.Repeat("c", 64), OpenIDEncrypted: "enc-c", Nickname: "C", Status: userModel.UserStatusDisabled, Version: 1, RegisteredAt: now},
	}
	...
	labels := make([]aiRequest.ClientFeatureLabelInput, 0, len(workspace.Config.FeatureLabels))
	for _, label := range workspace.Config.FeatureLabels {
		labels = append(labels, aiRequest.ClientFeatureLabelInput{
			Code:        label.Code,
			Title:       label.Title,
	...
	updated, replayed, err := service.UpdatePlatformPolicy(
		...
		aiRequest.PlatformPolicyUpdateInput{
	...
	var persisted aiModel.PlatformCapabilityPolicy
	if err := db.First(&persisted, "singleton_key = ?", serviceCommon.PlatformPolicySingletonKey).Error; err != nil {
	...
	emergency, _, err := service.SetPlatformEmergencyStatus(
		...
		aiTestActor(),
		"platform-emergency",
		aiRequest.PlatformPolicyEmergencyInput{
	...
	if err := db.Model(&aiModel.PlatformCapabilityPolicy{}).
...
func TestAIPlatformPolicyRejectsUnreadyEnableDuringEmergency(t *testing.T) {
	...
	labels := make([]aiRequest.ClientFeatureLabelInput, 0, len(emergency.FeatureLabels))
...
func TestAIProviderModelCapabilityAndPromptSaveApplyImmediately(t *testing.T) {
	...
	provider, _, err := service.CreateAIProvider(
		...
		aiTestActor(),
		"provider-create",
		aiRequest.AIProviderCreateInput{
			Name:      "DeepSeek",
			Type:      aiModel.AIProviderDeepSeek,
	...
	connectionTest, _, err := service.TestAIProviderConnection(
		...
		aiRequest.AIProviderConnectionTestInput{
			ExpectedVersion: provider.Version,
		},
	)
	...
	model, _, err := service.CreateAIModel(
		...
		aiTestActor(),
		"model-create",
		aiRequest.AIModelCreateInput{
			ProviderID:   provider.ID,
			Name:         "DeepSeek Chat",
			ModelKey:     "deepseek-chat",
			Capabilities: []aiModel.AIModelCapability{aiModel.AIModelText},
		},
	)
	if err != nil {
	...
	config, _, err := service.UpdateAICapability(
		...
		"capability-update",
		aiRequest.AICapabilityUpdateInput{
	...
	validation, _, err := service.ValidateAICapabilityPrompt(
		...
		aiRequest.AIPromptValidationInput{
			Prompt: aiRequest.AIPromptConfigInput{
				Mode:               aiModel.AIPromptCustom,
	...
	if err := db.Model(&aiModel.AICapabilityConfig{}).
		Where("capability_code = ?", aiModel.AICapabilityMealSuggest).
	...
	if err := db.Model(&aiModel.AIPromptDefaultConfig{}).
...
func TestAIProviderServiceRejectsDirectInvalidInputs(t *testing.T) {
	...
	_, err = service.ListAIProviders(
		context.Background(),
		aiRequest.AIProviderListQuery{SortOrder: "desc; DROP TABLE of_ai_providers"},
	)
	if appErrors.GetType(err) != appErrors.AdminBadRequest {
...
func TestAIModelServiceRejectsDirectInvalidInputs(t *testing.T) {
	...
	_, err = service.ListAIModels(
		context.Background(),
		aiRequest.AIModelListQuery{SortOrder: "desc; DROP TABLE of_ai_models"},
	)
	if appErrors.GetType(err) != appErrors.AdminBadRequest {
...
func TestAIMutationAuditFailureRollsBackMutationAndIdempotency(t *testing.T) {
	...
	_, _, err := service.CreateAIProvider(
		...
		aiRequest.AIProviderCreateInput{
			Name:      "千问",
			Type:      aiModel.AIProviderBailian,
	...
	if countErr := db.Model(&aiModel.AIProvider{}).Count(&providerCount).Error; countErr != nil {
		t.Fatalf("count providers: %v", countErr)
	}
	var idempotencyCount int64
	if countErr := db.Model(&commonModel.AdminIdempotencyRecord{}).
