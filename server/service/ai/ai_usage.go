...
func NewAIUsageService(
	...
	return &AIUsageService{
		DB: db, Permission: permission, Audit: serviceCommon.NewAccessAuditService(db),
...
func (service *AIUsageService) List(
	ctx context.Context,
	query aiRequest.AIUsageListQuery,
	actor commonRequest.AdminActor,
) (commonResponse.Page[aiResponse.AIUsageSummary], error) {
	query.ApplyDefaults()
	if err := validateAIUsageListQuery(query); err != nil {
		return commonResponse.Page[aiResponse.AIUsageSummary]{}, err
	}
	if service == nil || service.Permission == nil {
		return commonResponse.Page[aiResponse.AIUsageSummary]{}, appErrors.AdminInternal.DefaultMsg()
	}
	...
	statement := db.WithContext(ctx).Model(&aiModel.FrontFeatureUsage{})
	...
	if err := statement.Count(&total).Error; err != nil {
		return commonResponse.Page[aiResponse.AIUsageSummary]{},
	...
	if !ok {
		return commonResponse.Page[aiResponse.AIUsageSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	list := make([]aiResponse.AIUsageSummary, 0, len(rows))
...
func (service *AIUsageService) Detail(
	...
	actor commonRequest.AdminActor,
) (aiResponse.AIUsageDetail, error) {
	usageID = strings.TrimSpace(usageID)
	if usageID == "" || len([]rune(usageID)) > 64 {
		return aiResponse.AIUsageDetail{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	if service == nil || service.Permission == nil {
		return aiResponse.AIUsageDetail{}, appErrors.AdminInternal.DefaultMsg()
	}
	if err := service.Permission.Require(ctx, actor.AuthorityID, PermissionAIUsageRead); err != nil {
		return aiResponse.AIUsageDetail{}, err
	}
	if includeSensitive {
	...
	var row aiModel.FrontFeatureUsage
	if err := db.WithContext(ctx).First(&row, "id = ?", usageID).Error; err != nil {
		return aiResponse.AIUsageDetail{}, aiUsageLookupError(err)
	}
	...
	result := aiResponse.AIUsageDetail{
	...
	if includeSensitive && !row.SensitiveContentCleared {
		...
		if service.Audit == nil {
			service.Audit = serviceCommon.NewAccessAuditService(db)
		}
		if _, err := service.Audit.Record(ctx, serviceCommon.SensitiveAccess{
...
func (service *AIUsageService) ClearSensitiveContent(
	ctx context.Context,
	usageID string,
	input aiRequest.AIUsageSensitiveDeleteInput,
	actor commonRequest.AdminActor,
	idempotencyKey string,
) (aiResponse.AIUsageSensitiveDeleteResult, bool, error) {
	if service == nil || service.Permission == nil || service.Idempotency == nil {
		return aiResponse.AIUsageSensitiveDeleteResult{}, false, appErrors.AdminInternal.DefaultMsg()
	}
	usageID = strings.TrimSpace(usageID)
	if usageID == "" || len([]rune(usageID)) > 64 || input.ExpectedVersion < 1 {
		return aiResponse.AIUsageSensitiveDeleteResult{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	if err := serviceCommon.ValidateAdminActor(actor); err != nil {
		return aiResponse.AIUsageSensitiveDeleteResult{}, false, err
	}
	if err := serviceCommon.ValidateReason(input.Reason); err != nil {
		return aiResponse.AIUsageSensitiveDeleteResult{}, false, err
	}
	if err := service.Permission.Require(
		ctx,
	...
	payload := struct {
		...
		Input   aiRequest.AIUsageSensitiveDeleteInput `json:"input"`   // 清除参数
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			if err := (serviceCommon.GormMutationAuditWriter{}).WriteMutationAudit(ctx, tx, &audit); err != nil {
				return nil, err
			}
			return aiResponse.AIUsageSensitiveDeleteResult{
				Cleared: true, ClearedAt: now,
				RetainedAuditFields: append([]string(nil), aiUsageRetainedAuditFields...),
			}, nil
		},
	)
	return serviceCommon.DecodeIdempotentResult[aiResponse.AIUsageSensitiveDeleteResult](raw, replayed, err)
}
...
func (service *AIUsageService) summary(
	ctx context.Context,
	db *gorm.DB,
	row aiModel.FrontFeatureUsage,
) (aiResponse.AIUsageSummary, error) {
	var user userModel.MiniAppUser
	if err := db.WithContext(ctx).First(&user, "id = ?", row.UserID).Error; err != nil {
		return aiResponse.AIUsageSummary{}, aiUsageLookupError(err)
	}
	...
	if promptMode == "" || promptMode == string(aiModel.AIPromptPreset) {
		promptMode = "default"
	}
	return aiResponse.AIUsageSummary{
		ID: row.ID, User: serviceCommon.UserReference(user), CapabilityCode: capabilityCode,
...
func usagePointEntries(
	...
	db *gorm.DB,
	usageID string,
) ([]engagementModel.FrontPointEntry, error) {
...
func usagePointEntryIDs(rows []engagementModel.FrontPointEntry) (*string, *string) {
	var chargedID *string
	var refundID *string
	for index := range rows {
		value := rows[index].ID
		switch rows[index].Type {
		case engagementModel.PointSpent:
			chargedID = &value
		case engagementModel.PointRefund:
			refundID = &value
		}
	}
...
func aiUsageTimeline(
	row aiModel.FrontFeatureUsage,
	pointEntries []engagementModel.FrontPointEntry,
) []aiResponse.AIUsageTimelineEvent {
	timeline := []aiResponse.AIUsageTimelineEvent{{
		Type: "created", Label: "创建调用", OccurredAt: row.CreatedAt,
	}}
	for index := range pointEntries {
		switch pointEntries[index].Type {
		case engagementModel.PointSpent:
			timeline = append(timeline, aiResponse.AIUsageTimelineEvent{
	...
	if row.FinishedAt != nil {
		...
		case aiModel.FeatureExecutionSucceeded:
			label = "调用成功"
			eventType = "execution_succeeded"
		case aiModel.FeatureExecutionFailed:
			label = "调用失败"
	...
	if row.BillingStatus == aiModel.FeatureBillingRefundPending {
		timeline = append(timeline, aiResponse.AIUsageTimelineEvent{
...
func validateAIUsageListQuery(query aiRequest.AIUsageListQuery) error {
	if query.Page < 1 ||
