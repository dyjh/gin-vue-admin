...
func validateModelCapabilities(
	capabilities []aiModel.AIModelCapability,
) ([]string, error) {
	if len(capabilities) == 0 {
		return nil, appErrors.AdminInvalidConfig.DefaultMsg()
	}
	seen := make(map[aiModel.AIModelCapability]struct{}, len(capabilities))
	result := make([]string, 0, len(capabilities))
	for _, capability := range capabilities {
		switch capability {
		case aiModel.AIModelText,
			aiModel.AIModelVision,
			aiModel.AIModelImageGeneration:
		default:
			return nil, appErrors.AdminInvalidConfig.DefaultMsg()
		}
...
func modelSupports(
	model aiModel.AIModel,
	required aiModel.AIModelCapability,
) (bool, error) {
...
func modelReferencedCapabilityCount(db *gorm.DB, modelID string) (int64, error) {
	var count int64
	err := db.Model(&aiModel.AICapabilityConfig{}).
...
func modelCapabilityReferences(
	db *gorm.DB,
	modelID string,
) ([]aiResponse.AIModelCapabilityReference, error) {
	references := make([]aiResponse.AIModelCapabilityReference, 0)
...
func modelSummary(
	db *gorm.DB,
	model aiModel.AIModel,
) (aiResponse.AIModelSummary, error) {
	var provider aiModel.AIProvider
	if err := db.First(&provider, "id = ?", model.ProviderID).Error; err != nil {
		return aiResponse.AIModelSummary{}, appErrors.AdminInternal.Wrap(err, "get model provider")
	}
	providerResponse, err := providerSummary(db, provider)
	if err != nil {
		return aiResponse.AIModelSummary{}, err
	}
	capabilities, err := decodeModelCapabilities(model)
	if err != nil {
		return aiResponse.AIModelSummary{}, err
	...
	return aiResponse.AIModelSummary{
		ID:                             model.ID,
...
func modelDetail(
	db *gorm.DB,
	model aiModel.AIModel,
) (aiResponse.AIModelDetail, error) {
	summary, err := modelSummary(db, model)
	if err != nil {
		return aiResponse.AIModelDetail{}, err
	}
	references, err := modelCapabilityReferences(db, model.ID)
	if err != nil {
		return aiResponse.AIModelDetail{}, err
	}
	return aiResponse.AIModelDetail{
...
func validateAIModelListQuery(query aiRequest.AIModelListQuery) error {
	if query.Page < 1 ||
	...
	if query.Capability != "" {
		if _, err := validateModelCapabilities([]aiModel.AIModelCapability{query.Capability}); err != nil {
			return appErrors.AdminBadRequest.DefaultMsg()
		}
	}
	return nil
...
func validateAIModelCommonInput(input aiRequest.AIModelCreateInput) error {
...
func validateModelInput(
	db *gorm.DB,
	input aiRequest.AIModelCreateInput,
...
func (service *AIService) ListAIModels(
	ctx context.Context,
	query aiRequest.AIModelListQuery,
) (commonResponse.Page[aiResponse.AIModelSummary], error) {
	query.ApplyDefaults()
	if err := validateAIModelListQuery(query); err != nil {
		return commonResponse.Page[aiResponse.AIModelSummary]{}, err
	}
	...
	if !ok {
		return commonResponse.Page[aiResponse.AIModelSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	statement := db.Model(&aiModel.AIModel{})
	if query.Keyword != "" {
	...
	if err := statement.Count(&total).Error; err != nil {
		return commonResponse.Page[aiResponse.AIModelSummary]{}, appErrors.AdminInternal.Wrap(err, "count AI models")
	}
	...
	if err := statement.Order(column + " " + order).
		...
		return commonResponse.Page[aiResponse.AIModelSummary]{}, appErrors.AdminInternal.Wrap(err, "list AI models")
	}
	list := make([]aiResponse.AIModelSummary, 0, len(models))
	...
	return commonResponse.Page[aiResponse.AIModelSummary]{
		Page:     query.Page,
...
func (service *AIService) AIModelDetail(
	...
	if err := db.First(&model, "id = ?", modelID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return aiResponse.AIModelDetail{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return aiResponse.AIModelDetail{}, appErrors.AdminInternal.Wrap(err, "get AI model")
	}
...
func (service *AIService) CreateAIModel(
	...
	input aiRequest.AIModelCreateInput,
) (aiResponse.AIModelDetail, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.AIModelDetail{}, false, err
	}
	...
	return serviceCommon.DecodeIdempotentResult[aiResponse.AIModelDetail](raw, replayed, err)
}
...
func (service *AIService) UpdateAIModel(
	...
	modelID string,
	idempotencyKey string,
	input aiRequest.AIModelUpdateInput,
) (aiResponse.AIModelDetail, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.AIModelDetail{}, false, err
	}
	if err := validateAIModelIdentity(modelID); err != nil || input.ExpectedVersion < 1 {
		return aiResponse.AIModelDetail{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	commonInput := aiRequest.AIModelCreateInput{
	...
	payload := struct {
		...
		Input   aiRequest.AIModelUpdateInput `json:"input"`
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			update := tx.Model(&aiModel.AIModel{}).
...
func modelReferenceCount(db *gorm.DB, modelID string) (int64, error) {
	...
	if err := db.Model(&aiModel.AICapabilityConfig{}).
		Where("primary_model_id = ? OR auxiliary_model_id = ?", modelID, modelID).
