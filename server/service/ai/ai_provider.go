...
func providerModelCount(db *gorm.DB, providerID string) (int64, error) {
	var count int64
	if err := db.Model(&aiModel.AIModel{}).
...
func providerLastConnectionTest(
...
) *aiResponse.ProviderConnectionTestResult {
	...
	return &aiResponse.ProviderConnectionTestResult{
...
func providerReferencedCapabilityCount(db *gorm.DB, providerID string) (int64, error) {
	var count int64
	err := db.Table(aiModel.AICapabilityConfig{}.TableName()+" AS configs").
		Joins(
			"JOIN "+aiModel.AIModel{}.TableName()+
...
func providerSummary(
	db *gorm.DB,
	provider aiModel.AIProvider,
) (aiResponse.AIProviderSummary, error) {
	modelCount, err := providerModelCount(db, provider.ID)
	if err != nil {
		return aiResponse.AIProviderSummary{}, err
	}
	...
	return aiResponse.AIProviderSummary{
		ID:                     provider.ID,
		Name:                   provider.Name,
...
func providerReferenceDetails(
	db *gorm.DB,
	providerID string,
) ([]aiResponse.AIProviderModelReference, []aiResponse.AIProviderCapabilityReference, error) {
	...
	modelReferences := make([]aiResponse.AIProviderModelReference, 0, len(models))
	for index := range models {
		modelReferences = append(modelReferences, aiResponse.AIProviderModelReference{
			ID:       models[index].ID,
	...
	capabilityReferences := make([]aiResponse.AIProviderCapabilityReference, 0)
	if len(models) == 0 {
		return modelReferences, capabilityReferences, nil
	}
	if err := db.Table(aiModel.AICapabilityConfig{}.TableName()+" AS configs").
		Select(
		...
		Joins(
			"LEFT JOIN "+aiModel.AICapabilityDefinition{}.TableName()+
...
func validateAIProviderListQuery(query aiRequest.AIProviderListQuery) error {
	if query.Page < 1 ||
...
func (service *AIService) ListAIProviders(
	ctx context.Context,
	query aiRequest.AIProviderListQuery,
) (commonResponse.Page[aiResponse.AIProviderSummary], error) {
	query.ApplyDefaults()
	if err := validateAIProviderListQuery(query); err != nil {
		return commonResponse.Page[aiResponse.AIProviderSummary]{}, err
	}
	...
	if !ok {
		return commonResponse.Page[aiResponse.AIProviderSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	statement := db.Model(&aiModel.AIProvider{}).Where(
		"type IN ?",
		[]aiModel.AIProviderType{
			aiModel.AIProviderBailian,
	...
	if err := statement.Count(&total).Error; err != nil {
		return commonResponse.Page[aiResponse.AIProviderSummary]{}, appErrors.AdminInternal.Wrap(err, "count AI providers")
	}
	var providers []aiModel.AIProvider
	if err := statement.Order(column + " " + order).
		Order("id asc").
		Limit(query.PageSize).
		Offset((query.Page - 1) * query.PageSize).
		Find(&providers).Error; err != nil {
		return commonResponse.Page[aiResponse.AIProviderSummary]{}, appErrors.AdminInternal.Wrap(err, "list AI providers")
	}
	list := make([]aiResponse.AIProviderSummary, 0, len(providers))
	...
	return commonResponse.Page[aiResponse.AIProviderSummary]{
		Page:     query.Page,
...
func (service *AIService) AIProviderDetail(
	...
	if err := validateAIProviderIdentity(providerID); err != nil {
		return aiResponse.AIProviderDetail{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	if err := db.First(&provider, "id = ?", providerID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return aiResponse.AIProviderDetail{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return aiResponse.AIProviderDetail{}, appErrors.AdminInternal.Wrap(err, "get AI provider")
	}
...
func (service *AIService) AIProviderModelOptions(
	ctx context.Context,
	providerID string,
) ([]aiResponse.AIProviderModelOption, error) {
	if err := validateAIProviderIdentity(providerID); err != nil {
	...
	options := make([]aiResponse.AIProviderModelOption, 0, len(modelKeys))
	for _, modelKey := range modelKeys {
		_, exists := configured[modelKey]
		options = append(options, aiResponse.AIProviderModelOption{
			Name:       modelKey,
			ModelKey:   modelKey,
			Configured: exists,
		})
...
func (service *AIService) AIModelProviderOptions(
	ctx context.Context,
) ([]aiResponse.AIModelProviderOption, error) {
	var providers []aiModel.AIProvider
	...
	options := make([]aiResponse.AIModelProviderOption, 0, len(providers))
	for _, provider := range providers {
		options = append(options, aiResponse.AIModelProviderOption{
			ID:      provider.ID,
			Name:    provider.Name,
			Type:    string(provider.Type),
			Enabled: provider.Enabled,
...
func (service *AIService) CreateAIProvider(
	...
	input aiRequest.AIProviderCreateInput,
) (aiResponse.AIProviderDetail, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.AIProviderDetail{}, false, err
	}
	...
	if len([]rune(strings.TrimSpace(input.APIKey))) > 500 {
		return aiResponse.AIProviderDetail{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			var existing int64
			if err := tx.Model(&aiModel.AIProvider{}).
	...
	return serviceCommon.DecodeIdempotentResult[aiResponse.AIProviderDetail](raw, replayed, err)
}
...
func (service *AIService) UpdateAIProvider(
	...
	providerID string,
	idempotencyKey string,
	input aiRequest.AIProviderUpdateInput,
) (aiResponse.AIProviderDetail, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.AIProviderDetail{}, false, err
	}
	...
	payload := struct {
		...
		Input      aiRequest.AIProviderUpdateInput `json:"input"`
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			update := tx.Model(&aiModel.AIProvider{}).
...
func (service *AIService) UpdateAIProviderStatus(
	...
	providerID string,
	idempotencyKey string,
	input aiRequest.AIStatusUpdateInput,
) (aiResponse.AIProviderDetail, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.AIProviderDetail{}, false, err
	}
	...
	payload := struct {
		ProviderID string                        `json:"providerId"`
		Input      aiRequest.AIStatusUpdateInput `json:"input"`
	}{ProviderID: providerID, Input: input}
	raw, replayed, err := service.Idempotency.Execute(
		ctx,
		actor.AdministratorID,
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			if !*input.Enabled {
				...
				if err := tx.Model(&aiModel.AIModel{}).
...
func (service *AIService) DeleteAIProvider(
	...
	payload := struct {
		ProviderID string                  `json:"providerId"`
		Input      aiRequest.AIDeleteInput `json:"input"`
	}{ProviderID: providerID, Input: input}
	raw, replayed, err := service.Idempotency.Execute(
		ctx,
		actor.AdministratorID,
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			after := aiResponse.DeletedResult{Deleted: true}
			if err := service.writeMutationAudit(
				ctx,
				tx,
				actor,
...
func (service *AIService) TestAIProviderConnection(
	...
	providerID string,
	idempotencyKey string,
	input aiRequest.AIProviderConnectionTestInput,
) (aiResponse.ProviderConnectionTestResult, bool, error) {
	if err := validateAIActor(actor); err != nil {
		return aiResponse.ProviderConnectionTestResult{}, false, err
	}
	if err := validateAIProviderIdentity(providerID); err != nil ||
		input.ExpectedVersion < 1 ||
		(input.TimeoutMS != nil && (*input.TimeoutMS < 1000 || *input.TimeoutMS > 30000)) {
		return aiResponse.ProviderConnectionTestResult{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	providerID = strings.TrimSpace(providerID)
	payload := struct {
		ProviderID string                                  `json:"providerId"`
		Input      aiRequest.AIProviderConnectionTestInput `json:"input"`
	}{ProviderID: providerID, Input: input}
	raw, replayed, err := service.Idempotency.Execute(
		ctx,
		actor.AdministratorID,
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			after := aiResponse.ProviderConnectionTestResult{
	...
	return serviceCommon.DecodeIdempotentResult[aiResponse.ProviderConnectionTestResult](raw, replayed, err)
}
