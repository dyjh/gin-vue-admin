...
func TestAIUsageListReturnsActualProviderAndModelIDs(t *testing.T) {
	...
	usage := aiModel.FrontFeatureUsage{
		...
		PointCost:       2,
		ExecutionStatus: aiModel.FeatureExecutionSucceeded,
...
func TestAIUsageDetailSeparatesSensitiveContentAndBuildsTimeline(t *testing.T) {
	...
	user := userModel.MiniAppUser{
		...
		Status: userModel.UserStatusNormal, Version: 1, RegisteredAt: createdAt,
	...
	usage := aiModel.FrontFeatureUsage{
		...
		ExecutionStatus: aiModel.FeatureExecutionFailed,
		BillingStatus:   aiModel.FeatureBillingRefunded,
	...
	for _, entry := range []engagementModel.FrontPointEntry{
		{
			ID: "ai-usage-charge-entry", UserID: user.ID, Type: engagementModel.PointSpent,
			Scene: "feature_usage", Title: "扣除积分", Description: "AI调用",
			Amount: -2, RelatedObjectType: &objectType, RelatedObjectID: &usage.ID,
			CreatedAt: createdAt.Add(time.Second),
		},
		{
			ID: "ai-usage-refund-entry", UserID: user.ID, Type: engagementModel.PointRefund,
	...
	actor := commonRequest.AdminActor{
		AdministratorID: 1, Username: "admin", AuthorityID: commonModel.AuthorityOrderFoodSuperAdmin,
	...
	if err := db.Model(&commonModel.AdminAccessAudit{}).
...
func TestAIUsageClearSensitiveContentIsPermanentAndIdempotent(t *testing.T) {
	...
	user := userModel.MiniAppUser{
		...
		Status: userModel.UserStatusNormal, Version: 1, RegisteredAt: now,
	...
	var stored aiModel.FrontFeatureUsage
...
func TestAIUsageListRejectsUnsafeDirectSort(t *testing.T) {
	...
	_, err := service.List(
		context.Background(),
		aiRequest.AIUsageListQuery{SortOrder: "desc; DROP TABLE of_ai_usages"},
		commonRequest.AdminActor{AuthorityID: commonModel.AuthorityOrderFoodSuperAdmin},
	)
	if err == nil {
