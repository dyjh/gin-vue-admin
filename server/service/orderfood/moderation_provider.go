...
type ModerationProviderConfig struct {
	Provider       orderfoodModel.ModerationProviderType // 供应商类型
	Region         string                                // 接入地域
...
type ModerationProviderImage struct {
	...
	Scene       orderfoodModel.ModerationScene // 业务审核场景
}

// ModerationProviderResult 表示供应商审核结果归一化后的内部数据。
type ModerationProviderResult struct {
	Status                  orderfoodModel.ModerationStatus // 平台审核状态
...
func (provider *AliyunModerationProvider) TestConnection(
	...
	startedAt := time.Now()
	if config.Provider != orderfoodModel.ModerationProviderAliyun {
	...
	callResult.Status = orderfoodModel.ModerationStatusPassed
...
func (provider *AliyunModerationProvider) ModerateImage(
	...
	case "review", "block":
		callResult.Status = orderfoodModel.ModerationStatusRejected
		callResult.Category = ModerationCategoryContentRisk
		callResult.SafeMessage = "图片内容审核未通过"
	default:
		callResult.Status = orderfoodModel.ModerationStatusFailed
...
func moderationFailedProviderResult(category string, safeMessage string, durationMS int) ModerationProviderResult {
	return ModerationProviderResult{
		Status:      orderfoodModel.ModerationStatusFailed,
		Category:    category,
		RiskLabels:  []string{},
