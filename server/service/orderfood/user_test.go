...
func testAdminActor() orderfoodRequest.AdminActor {
...
func TestPreferenceProfileFailsClosedWhenAuditWriteFails(t *testing.T) {
	...
	service := NewUserService(db, NewAccessAuditService(db), nil, nil)
	...
	if err := db.Model(&orderfoodModel.AdminAccessAudit{}).Count(&accessAuditCount).Error; err != nil {
...
func TestUserStatusOptimisticLockAndIdempotency(t *testing.T) {
	...
	meal := orderfoodModel.FrontMeal{
		...
		Code: "654321", Status: orderfoodModel.MealConfirmed,
	...
	if err := db.Create(&orderfoodModel.FrontMealFinalDish{
	...
	if err := db.Create(&orderfoodModel.FrontShoppingList{
	...
	if err := db.Create(&orderfoodModel.MealFinalResultSubscription{
	...
	service := NewUserService(db, nil, NewIdempotencyService(db), nil)
	...
	body := orderfoodRequest.UpdateUserStatusBody{
		Status:          orderfoodModel.UserStatusDisabled,
		Reason:          "违反平台规则",
		ExpectedVersion: 1,
	}
	...
	if replayed || first.Version != 2 || first.Status != string(orderfoodModel.UserStatusDisabled) {
	...
	conflictingBody := orderfoodRequest.UpdateUserStatusBody{
		Status:          orderfoodModel.UserStatusNormal,
		Reason:          "人工审核恢复",
		ExpectedVersion: 2,
	}
	...
	if subscription.ConsumedResult == nil ||
		*subscription.ConsumedResult != string(orderfoodModel.MealCancelled) ||
	...
	var audits []orderfoodModel.AdminAuditLog
...
func TestUserStatusAuditFailureRollsBackMutationAndIdempotency(t *testing.T) {
	...
	_, _, err := NewUserService(db, nil, NewIdempotencyService(db), nil).UpdateStatus(
		context.Background(),
		testAdminActor(),
		user.ID,
		"status-audit-failure",
		orderfoodRequest.UpdateUserStatusBody{
			Status:          orderfoodModel.UserStatusDisabled,
			Reason:          "审计写入失败",
			ExpectedVersion: 1,
		},
	...
	if stored.Status != orderfoodModel.UserStatusNormal || stored.Version != 1 {
		t.Fatalf("user mutation was not rolled back: status=%s version=%d", stored.Status, stored.Version)
	}
	var idempotencyCount int64
	if err := db.Model(&orderfoodModel.AdminIdempotencyRecord{}).Count(&idempotencyCount).Error; err != nil {
...
func TestUserListFiltersShanghaiActivityDaysMySQL(t *testing.T) {
	...
	activity := orderfoodModel.FrontUserActivityDay{
	...
	list, err := NewUserService(db, nil, nil, StaticCapabilityPolicyProvider{}).List(
		context.Background(),
		orderfoodRequest.UserListQuery{
...
func TestIdempotencyProcessingAndPayloadConflict(t *testing.T) {
	db := newOrderFoodTestDB(t)
	if err := db.AutoMigrate(&orderfoodModel.AdminIdempotencyRecord{}); err != nil {
	...
	now := time.Now().UTC()
	record := orderfoodModel.AdminIdempotencyRecord{
		AdministratorID: 8,
		EndpointID:      "user_status_update",
		IdempotencyKey:  "processing-user-001",
		RequestHash:     requestHash,
		State:           orderfoodModel.AdminIdempotencyProcessing,
		CreatedAt:       now,
		UpdatedAt:       now,
	...
	actionCalled := false
	_, _, err = NewIdempotencyService(db).Execute(
		context.Background(),
		8,
		"user_status_update",
		"processing-user-001",
