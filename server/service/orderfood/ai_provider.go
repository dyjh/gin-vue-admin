...
func providerSummary(
	...
	configured := strings.TrimSpace(provider.CredentialRef) != ""
	if configured {
		label := "已配置（安全引用）"
		maskedLabel = &label
	}
...
func (service *AIService) CreateAIProvider(
	...
	if len([]rune(strings.TrimSpace(input.CredentialRef))) > 300 {
...
func (service *AIService) UpdateAIProvider(
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			if input.CredentialRef != nil {
				nextCredentialRef := strings.TrimSpace(*input.CredentialRef)
				connectionConfigChanged = connectionConfigChanged ||
					provider.CredentialRef != nextCredentialRef
				provider.CredentialRef = nextCredentialRef
			}
			provider.TimeoutMS = input.TimeoutMS
