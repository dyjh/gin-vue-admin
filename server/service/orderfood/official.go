...
func validateOfficialDishActor(actor orderfoodRequest.AdminActor) error {
...
// officialDishAdministrator 将管理员持久化字段转换为响应摘要。
func officialDishAdministrator(
	id uint,
	username string,
	nickname *string,
...
func normalizeOfficialDishInput(
...
) (orderfoodRequest.OfficialDishCreateInput, error) {
	...
	if input.Name == "" || utf8.RuneCountInString(input.Name) > 40 ||
		...
		(input.Status != orderfoodModel.DishStatusDraft &&
			input.Status != orderfoodModel.DishStatusUsable) ||
...
type officialDishReferences struct {
	Category orderfoodModel.ContentCategory        // 分类
	Tags     []orderfoodModel.ContentTag           // 标签列表
	Units    map[string]orderfoodModel.ContentUnit // 单位映射
	Cover    orderfoodModel.FrontMediaAsset        // 官方封面
}
...
func (service *OfficialDishService) loadOfficialDishReferences(
	...
	result := officialDishReferences{
		Tags:  make([]orderfoodModel.ContentTag, 0, len(input.TagIDs)),
	...
	if err := db.WithContext(ctx).
		First(&result.Cover, "id = ? AND upload_source = ? AND scene = ? AND resource_status = ? AND review_status = ?",
			...
			mediaResourceActive,
			orderfoodModel.MediaReviewNotRequired,
		).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func preloadOfficialDish(
	db *gorm.DB,
	dishID uint,
) (orderfoodModel.OfficialDish, error) {
...
func officialDishCopyCount(
	...
	err := db.WithContext(ctx).
		Table((orderfoodModel.RecommendationCopy{}).TableName()+" AS copies").
		Joins("JOIN "+(orderfoodModel.PlatformRecommendation{}).TableName()+
...
func officialDishSummary(
	dish orderfoodModel.OfficialDish,
	copyCount int64,
) orderfoodResponse.OfficialDishSummary {
	...
	return orderfoodResponse.OfficialDishSummary{
		ID:       dish.PublicID,
		Name:     dish.Name,
		...
		UpdatedBy: officialDishAdministrator(
...
func officialDishDetail(
	dish orderfoodModel.OfficialDish,
	copyCount int64,
) orderfoodResponse.OfficialDishDetail {
...
func officialDishOnlineRecommendationCount(
	...
	var count int64
	if err := db.WithContext(ctx).
		Model(&orderfoodModel.PlatformRecommendation{}).
		Where(
			"source_type = ? AND official_dish_id = ? AND status = ?",
			recommendationSourceOfficial,
			dishID,
			recommendationStatusPublished,
		).
...
func (service *OfficialDishService) loadOfficialDishDetail(
	...
	db *gorm.DB,
	dishID uint,
) (orderfoodResponse.OfficialDishDetail, error) {
	dish, err := preloadOfficialDish(db.WithContext(ctx), dishID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return orderfoodResponse.OfficialDishDetail{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return orderfoodResponse.OfficialDishDetail{}, appErrors.AdminInternal.Wrap(err, "load official dish detail")
	}
	copyCount, err := officialDishCopyCount(ctx, db, dish.ID)
	if err != nil {
		return orderfoodResponse.OfficialDishDetail{}, err
	}
...
func (service *OfficialDishService) ListOfficialDishes(
	ctx context.Context,
	query orderfoodRequest.OfficialDishListQuery,
) (orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary], error) {
	query.ApplyDefaults()
	if !validPagination(query.Page, query.PageSize) ||
		!validTimeRange(query.CreatedFrom, query.CreatedTo) ||
		utf8.RuneCountInString(query.Keyword) > 60 ||
		len(query.TagIDs) > 3 ||
		(query.SortOrder != "asc" && query.SortOrder != "desc") {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	db := service.database()
	if db == nil {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{}, appErrors.AdminInternal.DefaultMsg()
	}
	statement := db.WithContext(ctx).Model(&orderfoodModel.OfficialDish{})
	...
	if len(query.TagIDs) > 0 {
		statement = statement.Where(
			"of_off_dishes.id IN (?)",
			db.Table((orderfoodModel.OfficialDishTag{}).TableName()+" AS dish_tags").
				Joins("JOIN "+(orderfoodModel.ContentTag{}).TableName()+
	...
	if err := statement.Count(&total).Error; err != nil {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{}, appErrors.AdminInternal.Wrap(err, "count official dishes")
	}
	...
	rows := make([]orderfoodModel.OfficialDish, 0)
	if err := statement.
		Preload("Category").
		Preload("Tags").
		...
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{}, appErrors.AdminInternal.Wrap(err, "list official dishes")
	}
	list := make([]orderfoodResponse.OfficialDishSummary, 0, len(rows))
	for _, dish := range rows {
		copyCount, err := officialDishCopyCount(ctx, db, dish.ID)
		if err != nil {
			return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{}, err
		}
	...
	return orderfoodResponse.Page[orderfoodResponse.OfficialDishSummary]{
...
func (service *OfficialDishService) GetOfficialDish(
	...
	if db == nil || publicID == "" {
		return orderfoodResponse.OfficialDishDetail{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	var dish orderfoodModel.OfficialDish
	if err := db.WithContext(ctx).First(&dish, "public_id = ?", publicID).Error; err != nil {
		...
		return orderfoodResponse.OfficialDishDetail{}, appErrors.AdminInternal.Wrap(err, "load official dish")
	}
...
func persistOfficialDishParts(
	...
	if err := tx.Unscoped().Where("dish_id = ?", dishID).
		Delete(&orderfoodModel.OfficialDishIngredient{}).Error; err != nil {
		return appErrors.AdminInternal.Wrap(err, "replace official dish ingredients")
	}
	if err := tx.Unscoped().Where("dish_id = ?", dishID).
		Delete(&orderfoodModel.OfficialDishStep{}).Error; err != nil {
		return appErrors.AdminInternal.Wrap(err, "replace official dish steps")
	}
	if err := tx.Where("dish_id = ?", dishID).
		Delete(&orderfoodModel.OfficialDishTag{}).Error; err != nil {
		return appErrors.AdminInternal.Wrap(err, "replace official dish tags")
	}
	ingredients := make([]orderfoodModel.OfficialDishIngredient, 0, len(input.Ingredients))
	for _, item := range input.Ingredients {
		...
		ingredients = append(ingredients, orderfoodModel.OfficialDishIngredient{
	...
	steps := make([]orderfoodModel.OfficialDishStep, 0, len(input.Steps))
	for _, item := range input.Steps {
		steps = append(steps, orderfoodModel.OfficialDishStep{
	...
	if len(references.Tags) > 0 {
		joins := make([]orderfoodModel.OfficialDishTag, 0, len(references.Tags))
		now := time.Now().UTC()
		for _, tag := range references.Tags {
			joins = append(joins, orderfoodModel.OfficialDishTag{
...
func (service *OfficialDishService) CreateOfficialDish(
	...
	idempotencyKey string,
	input orderfoodRequest.OfficialDishCreateInput,
) (orderfoodResponse.OfficialDishDetail, bool, error) {
	if err := validateOfficialDishActor(actor); err != nil {
		return orderfoodResponse.OfficialDishDetail{}, false, err
	}
	...
	var result orderfoodResponse.OfficialDishDetail
...
func (service *OfficialDishService) offlineOfficialDishRecommendations(
	...
	now := service.now()
	update := tx.Model(&orderfoodModel.PlatformRecommendation{}).
		Where("source_type = ? AND official_dish_id = ? AND status = ?",
			recommendationSourceOfficial, dishID, recommendationStatusPublished).
...
func (service *OfficialDishService) UpdateOfficialDish(
	...
	actor orderfoodRequest.AdminActor,
	idempotencyKey string,
	input orderfoodRequest.OfficialDishUpdateInput,
) (orderfoodResponse.OfficialDishMutationResult, bool, error) {
	if err := validateOfficialDishActor(actor); err != nil {
		return orderfoodResponse.OfficialDishMutationResult{}, false, err
	}
	publicID = strings.TrimSpace(publicID)
	normalized, err := normalizeOfficialDishInput(input.OfficialDishCreateInput)
	if err != nil || publicID == "" || input.ExpectedVersion < 1 {
		return orderfoodResponse.OfficialDishMutationResult{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	payload := struct {
		ID              string                                   `json:"id"`              // 官方菜品ID
		Input           orderfoodRequest.OfficialDishCreateInput `json:"input"`           // 菜品内容
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			now := service.now()
			update := tx.Model(&orderfoodModel.OfficialDish{}).
			...
			result := orderfoodResponse.OfficialDishMutationResult{
	...
	var result orderfoodResponse.OfficialDishMutationResult
	if err := json.Unmarshal(raw, &result); err != nil {
...
func (service *OfficialDishService) DeleteOfficialDish(
	...
	actor orderfoodRequest.AdminActor,
	idempotencyKey string,
	input orderfoodRequest.OfficialDishDeleteInput,
) (orderfoodResponse.OfficialDishDeleteResult, bool, error) {
	if err := validateOfficialDishActor(actor); err != nil {
		return orderfoodResponse.OfficialDishDeleteResult{}, false, err
	}
	publicID = strings.TrimSpace(publicID)
	...
	if publicID == "" || input.ExpectedVersion < 1 ||
		...
		return orderfoodResponse.OfficialDishDeleteResult{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	payload := struct {
		ID    string                                   `json:"id"`    // 官方菜品ID
		Input orderfoodRequest.OfficialDishDeleteInput `json:"input"` // 删除参数
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			result := orderfoodResponse.OfficialDishDeleteResult{
	...
	var result orderfoodResponse.OfficialDishDeleteResult
	if err := json.Unmarshal(raw, &result); err != nil {
...
func officialDishCoverSummary(
...
) orderfoodResponse.OfficialDishCoverSummary {
	...
	return orderfoodResponse.OfficialDishCoverSummary{
		FileID:         asset.ID,
		URL:            asset.URL,
		...
		UploadedBy: officialDishAdministrator(
...
func (service *OfficialDishService) ListOfficialDishCovers(
	ctx context.Context,
	query orderfoodRequest.OfficialDishCoverListQuery,
) (orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary], error) {
	query.ApplyDefaults()
	db := service.database()
	if db == nil {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary]{}, appErrors.AdminInternal.DefaultMsg()
	}
	statement := db.WithContext(ctx).Model(&orderfoodModel.FrontMediaAsset{}).
		Where("upload_source = ? AND scene = ? AND resource_status = ?",
			officialDishUploadSource, officialDishCoverScene, mediaResourceActive)
	if keyword := strings.TrimSpace(query.Keyword); keyword != "" {
		statement = statement.Where("file_name LIKE ?", "%"+keyword+"%")
	}
	var total int64
	if err := statement.Count(&total).Error; err != nil {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary]{}, appErrors.AdminInternal.Wrap(err, "count official dish covers")
	}
	sortColumns := map[string]string{
	...
	column, ok := sortColumns[query.SortBy]
	if !ok {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	...
	if err := statement.Order(column + " " + query.SortOrder).
		...
		Offset((query.Page - 1) * query.PageSize).
		Find(&rows).Error; err != nil {
		return orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary]{}, appErrors.AdminInternal.Wrap(err, "list official dish covers")
	}
	list := make([]orderfoodResponse.OfficialDishCoverSummary, 0, len(rows))
	for _, asset := range rows {
		list = append(list, officialDishCoverSummary(asset))
	}
	return orderfoodResponse.Page[orderfoodResponse.OfficialDishCoverSummary]{
		Page: query.Page, PageSize: query.PageSize, Total: total, List: list,
	}, nil
}
...
func (service *OfficialDishService) UploadOfficialDishCover(
	...
	contentType string,
	payload []byte,
) (orderfoodResponse.OfficialDishCoverSummary, bool, error) {
	if err := validateOfficialDishActor(actor); err != nil {
		return orderfoodResponse.OfficialDishCoverSummary{}, false, err
	}
	...
	if fileName == "" || len(payload) == 0 || len(payload) > maxOfficialCoverBytes ||
		...
		return orderfoodResponse.OfficialDishCoverSummary{}, false, appErrors.AdminInvalidImage.DefaultMsg()
	}
	...
	var result orderfoodResponse.OfficialDishCoverSummary
	if err := json.Unmarshal(raw, &result); err != nil {
