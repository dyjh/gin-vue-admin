...
// TestValidateCredentialReferenceSupportsRuntimeResolverOnly 验证配置层只接受生产解析器可执行的环境变量引用。
func TestValidateCredentialReferenceSupportsRuntimeResolverOnly(t *testing.T) {
	if err := validateCredentialReference("env://DEEPSEEK_API_KEY"); err != nil {
		t.Fatalf("validate env reference: %v", err)
	}
	for _, reference := range []string{
		"secret://orderfood/deepseek",
		"env://",
		"env://1INVALID_NAME",
		"env://invalid/name",
		"DEEPSEEK_API_KEY",
		...
		if err := validateCredentialReference(reference); err == nil {
			t.Fatalf("expected reference to fail: %s", reference)
		}
	}
}

// TestEnvironmentAICredentialResolver 验证 AI 供应商环境变量引用能够读取且遵守上下文取消。
func TestEnvironmentAICredentialResolver(t *testing.T) {
	t.Setenv("ORDERFOOD_AI_PROVIDER_TEST_KEY", "test-api-key")
	resolver := EnvironmentAICredentialResolver{}
	value, err := resolver.Resolve(
		context.Background(),
		" env://ORDERFOOD_AI_PROVIDER_TEST_KEY ",
	)
	if err != nil {
		t.Fatalf("resolve AI credential: %v", err)
	}
	if value != "test-api-key" {
		t.Fatalf("unexpected AI credential: %q", value)
	}
...
func seedAllAICapabilitiesReady(t *testing.T, db *gorm.DB, now time.Time) {
	...
	provider := orderfoodModel.AIProvider{
		...
		CredentialRef: "env://AI_TEST_KEY", TimeoutMS: 30000,
...
func TestAIProviderModelCapabilityAndPromptSaveApplyImmediately(t *testing.T) {
	...
	if strings.Contains(string(encodedProvider), "DEEPSEEK_API_KEY") {
