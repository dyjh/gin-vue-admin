...
func (service *IdempotencyService) Execute(
	...
	err = db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var record orderfoodModel.AdminIdempotencyRecord
		findErr := tx.
			Where(
		...
		case findErr == nil && record.ExpiresAt.After(now):
			...
			switch record.State {
			case orderfoodModel.AdminIdempotencyProcessing:
				return appErrors.AdminRequestProcessing.DefaultMsg()
			case orderfoodModel.AdminIdempotencyCompleted:
		...
		update := tx.Model(&record).Updates(map[string]interface{}{
			"state":         orderfoodModel.AdminIdempotencyCompleted,
			"response_code": 0,
