...
func (service *GovernanceService) ListRecords(
	ctx context.Context,
	query orderfoodRequest.GovernanceRecordListQuery,
	actor orderfoodRequest.AdminActor,
) (orderfoodResponse.Page[orderfoodResponse.GovernanceRecordSummary], error) {
	query.ApplyDefaults()
	if err := validateGovernanceRecordListQuery(query); err != nil {
		return orderfoodResponse.Page[orderfoodResponse.GovernanceRecordSummary]{}, err
	}
	if service == nil || service.Permission == nil {
		return orderfoodResponse.Page[orderfoodResponse.GovernanceRecordSummary]{}, appErrors.AdminInternal.DefaultMsg()
	}
	...
	statement := db.WithContext(ctx).Model(&orderfoodModel.GovernanceRecord{})
	...
	if query.JobStatus == "abnormal" {
		statement = statement.Where(
			"job_status IN ?",
			[]string{
				orderfoodModel.GovernanceJobProcessing,
				orderfoodModel.GovernanceJobPartiallySucceeded,
				orderfoodModel.GovernanceJobFailed,
			},
		)
	...
	if value := strings.TrimSpace(query.AdministratorID); value != "" {
		...
		if err != nil || administratorID == 0 {
			return orderfoodResponse.Page[orderfoodResponse.GovernanceRecordSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
		}
	...
	if err := statement.Count(&total).Error; err != nil {
		return orderfoodResponse.Page[orderfoodResponse.GovernanceRecordSummary]{},
	...
	var records []orderfoodModel.GovernanceRecord
	if err := statement.
	...
	list := make([]orderfoodResponse.GovernanceRecordSummary, 0, len(records))
	...
	if len(recordIDs) > 0 {
		var jobs []orderfoodModel.GovernanceJob
		...
		progressByRecordID := make(map[string]orderfoodResponse.GovernanceJobProgress, len(jobs))
...
func validateGovernanceRecordListQuery(query orderfoodRequest.GovernanceRecordListQuery) error {
...
func (service *GovernanceService) GetRecord(
	...
	recordID string,
	actor orderfoodRequest.AdminActor,
) (orderfoodResponse.GovernanceRecordDetail, error) {
	if service == nil || service.Permission == nil {
		return orderfoodResponse.GovernanceRecordDetail{}, appErrors.AdminInternal.DefaultMsg()
	}
	if err := service.Permission.Require(ctx, actor.AuthorityID, PermissionGovernanceRead); err != nil {
		return orderfoodResponse.GovernanceRecordDetail{}, err
	}
	db := service.database()
	if db == nil {
		return orderfoodResponse.GovernanceRecordDetail{}, appErrors.AdminInternal.DefaultMsg()
	...
	if err := db.WithContext(ctx).First(&record, "public_id = ?", strings.TrimSpace(recordID)).Error; err != nil {
		return orderfoodResponse.GovernanceRecordDetail{}, governanceLookupError(err)
	}
...
func (service *GovernanceService) GetJob(
	...
	if service == nil || service.Permission == nil {
		return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.DefaultMsg()
	}
	if err := service.Permission.Require(ctx, actor.AuthorityID, PermissionGovernanceRead); err != nil {
		return orderfoodResponse.GovernanceJob{}, err
	}
	db := service.database()
	if db == nil {
		return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.DefaultMsg()
...
func (service *GovernanceService) ProcessPendingJobs(ctx context.Context, batchSize int) error {
	...
	var jobIDs []string
	if err := db.WithContext(ctx).Model(&orderfoodModel.GovernanceJob{}).
		Where("status IN ?", []string{
			orderfoodModel.GovernanceJobPending,
			orderfoodModel.GovernanceJobProcessing,
		}).
...
func (service *GovernanceService) processPendingJob(
	...
	return db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if job.Status != orderfoodModel.GovernanceJobPending &&
			job.Status != orderfoodModel.GovernanceJobProcessing {
			return nil
		}
		now := service.now()
		if job.Status == orderfoodModel.GovernanceJobPending {
			update := tx.Model(&orderfoodModel.GovernanceJob{}).
				Where("id = ? AND version = ?", job.ID, job.Version).
				Updates(map[string]interface{}{
					"status":     orderfoodModel.GovernanceJobProcessing,
					"started_at": now,
		...
		var items []orderfoodModel.GovernanceJobItem
		if err := tx.
			Where("job_id = ? AND status = ?", job.ID, orderfoodModel.GovernanceJobItemPending).
			Order("id ASC").
			Limit(batchSize).
		...
		for index := range items {
			...
			if err := service.processJobItem(ctx, tx, &items[index], record, now); err != nil {
				...
				if updateErr := tx.Model(&orderfoodModel.GovernanceJobItem{}).
					Where("id = ? AND status = ?", items[index].ID, orderfoodModel.GovernanceJobItemPending).
					Updates(map[string]interface{}{
						"status":             orderfoodModel.GovernanceJobItemFailed,
...
func (service *GovernanceService) RetryJob(
	ctx context.Context,
	jobID string,
	input orderfoodRequest.GovernanceJobRetryInput,
	actor orderfoodRequest.AdminActor,
	idempotencyKey string,
) (orderfoodResponse.GovernanceJob, bool, error) {
	var result orderfoodResponse.GovernanceJob
	...
	payload := struct {
		...
		Input orderfoodRequest.GovernanceJobRetryInput `json:"input"` // 重试参数
...
func (service *GovernanceService) retryJobOnce(
	...
	if job.Version != input.ExpectedVersion || job.FailedCount == 0 {
		return orderfoodResponse.GovernanceJob{}, appErrors.AdminStateConflict.DefaultMsg()
	}
	var record orderfoodModel.GovernanceRecord
	...
	if err := tx.WithContext(ctx).
		Where("job_id = ? AND status = ?", job.ID, orderfoodModel.GovernanceJobItemFailed).
		Order("id ASC").
		Find(&items).Error; err != nil {
		return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.Wrap(err, "list failed governance job items")
	}
	if len(items) == 0 {
		return orderfoodResponse.GovernanceJob{}, appErrors.AdminStateConflict.DefaultMsg()
	}
	...
	for index := range items {
		...
		if err := tx.SavePoint(savepoint).Error; err != nil {
			return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.Wrap(err, "create governance retry savepoint")
		}
		if err := service.processJobItem(ctx, tx, &items[index], record, now); err != nil {
			if rollbackErr := tx.RollbackTo(savepoint).Error; rollbackErr != nil {
				return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.Wrap(rollbackErr, "rollback governance retry item")
			}
			...
			if updateErr := tx.Model(&orderfoodModel.GovernanceJobItem{}).
				...
				return orderfoodResponse.GovernanceJob{}, appErrors.AdminInternal.Wrap(updateErr, "mark governance retry item failed")
			}
		}
	}
	...
	if service.MutationAudit == nil {
		service.MutationAudit = gormMutationAuditWriter{}
	}
...
func (service *GovernanceService) processJobItem(
	...
	if !dish.DeletedAt.Valid {
		...
		if err := tx.WithContext(ctx).Model(&orderfoodModel.PlatformRecommendation{}).
			...
			Updates(map[string]interface{}{
				"status": recommendationStatusOffline, "selected": false,
	...
	if notificationID == nil {
		...
		value := newPublicID("notice")
		notification := orderfoodModel.UserNotification{
	...
	return tx.WithContext(ctx).Model(&orderfoodModel.GovernanceJobItem{}).
		Where("id = ? AND status IN ?", item.ID, []string{
			orderfoodModel.GovernanceJobItemPending,
			orderfoodModel.GovernanceJobItemFailed,
		}).
		Updates(map[string]interface{}{
			"status": orderfoodModel.GovernanceJobItemSucceeded, "attempt_count": gorm.Expr("attempt_count + 1"),
...
func syncGovernanceRecordProgress(
	...
	if err := tx.WithContext(ctx).Model(&orderfoodModel.GovernanceJobItem{}).
		Where("job_id = ? AND status = ? AND notification_id IS NOT NULL", job.ID, orderfoodModel.GovernanceJobItemSucceeded).
		Order("id ASC").
...
func refreshGovernanceJobProgress(
	...
	for _, count := range counts {
		switch count.Status {
		case orderfoodModel.GovernanceJobItemSucceeded:
			job.SucceededCount = count.Count
		case orderfoodModel.GovernanceJobItemFailed:
	...
	case job.FailedCount == 0:
		job.Status = orderfoodModel.GovernanceJobSucceeded
		job.FinishedAt = &now
	case job.SucceededCount == 0:
		job.Status = orderfoodModel.GovernanceJobFailed
		job.FinishedAt = &now
	default:
		job.Status = orderfoodModel.GovernanceJobPartiallySucceeded
		job.FinishedAt = &now
	}
	...
	update := tx.WithContext(ctx).Model(&orderfoodModel.GovernanceJob{}).
...
func governanceRecordSummary(
	record orderfoodModel.GovernanceRecord,
) (orderfoodResponse.GovernanceRecordSummary, error) {
	var actions []string
	if err := json.Unmarshal(record.Actions, &actions); err != nil {
		return orderfoodResponse.GovernanceRecordSummary{},
...
func (service *GovernanceService) governanceRecordDetail(
	...
	var impact orderfoodResponse.GovernanceImpactPreview
	if err := json.Unmarshal(record.ImpactSnapshot, &impact); err != nil {
		return orderfoodResponse.GovernanceRecordDetail{},
	...
	var jobResponse *orderfoodResponse.GovernanceJob
	var job orderfoodModel.GovernanceJob
	if err := db.WithContext(ctx).First(&job, "record_id = ?", record.PublicID).Error; err == nil {
		...
		if detailErr != nil {
			return orderfoodResponse.GovernanceRecordDetail{}, detailErr
		}
		jobResponse = &value
...
func governanceTargetState(
	...
	var result *gorm.DB
	switch targetType {
	case orderfoodModel.GovernanceTargetDish:
		result = db.WithContext(ctx).Model(&orderfoodModel.UserDish{}).
			Select("version").Where("public_id = ?", targetID).Take(&state)
	case orderfoodModel.GovernanceTargetOfficialDish:
		result = db.WithContext(ctx).Model(&orderfoodModel.OfficialDish{}).
			Select("version").Where("public_id = ?", targetID).Take(&state)
	case orderfoodModel.GovernanceTargetRecipe:
		result = db.WithContext(ctx).Model(&orderfoodModel.UserRecipe{}).
			Select("version").Where("public_id = ?", targetID).Take(&state)
	case orderfoodModel.GovernanceTargetCheckin:
		result = db.WithContext(ctx).Model(&orderfoodModel.FrontCheckin{}).
...
func governanceJobResponse(job orderfoodModel.GovernanceJob) orderfoodResponse.GovernanceJob {
	return orderfoodResponse.GovernanceJob{
...
func governanceJobProgress(job orderfoodModel.GovernanceJob) orderfoodResponse.GovernanceJobProgress {
	return orderfoodResponse.GovernanceJobProgress{
		TotalCount: job.TotalCount, SucceededCount: job.SucceededCount,
		FailedCount: job.FailedCount, PendingCount: job.PendingCount,
	}
}
...
func governanceJobDetail(
	...
	var rows []orderfoodModel.GovernanceJobItem
	if err := db.WithContext(ctx).
		Where("job_id = ?", job.ID).
		Order("id ASC").
	...
	result.Items = make([]orderfoodResponse.GovernanceJobItem, 0, len(rows))
	for _, row := range rows {
		result.Items = append(result.Items, orderfoodResponse.GovernanceJobItem{
