...
func subscriptionTestActor() commonRequest.AdminActor {
...
func TestNotificationAdminReadOnlyQueryAndDetail(t *testing.T) {
	...
	targetType := "dish"
	targetID := "dish-001"
	rows := []engagementModel.UserNotification{
		{
	...
	result, err := service.ListNotifications(
		context.Background(),
		engagementRequest.NotificationAdminListQuery{
	...
	var persisted engagementModel.UserNotification
...
func TestSubscribeSceneFixedConfiguration(t *testing.T) {
	db := openSubscriptionTestDB(t)
	service := NewSubscriptionService(db, serviceCommon.NewIdempotencyService(db))
	...
	if err != nil || len(scenes) != 1 ||
		scenes[0].Scene != engagementModel.SubscribeSceneMealStatus ||
	...
	unconfigured, err := service.GetSubscribeScene(ctx, engagementModel.SubscribeSceneMealStatus)
	...
	configured, replayed, err := service.ConfigureSubscribeScene(
		...
		engagementModel.SubscribeSceneMealStatus,
		engagementRequest.SubscribeSceneConfigureInput{
	...
	if err := db.Model(&engagementModel.SubscribeMessageTemplate{}).
		Where("scene = ?", engagementModel.SubscribeSceneMealStatus).
	...
	enabled, replayed, err := service.UpdateSubscribeSceneStatus(
		...
		engagementModel.SubscribeSceneMealStatus,
		engagementRequest.SubscribeSceneStatusInput{
...
func TestSubscribeTemplateAndLogAdminFlow(t *testing.T) {
	...
	created, replayed, err := service.CreateSubscribeTemplate(
		...
		engagementRequest.SubscribeTemplateCreateInput{
			Name:             "饭局最终结果",
			WechatTemplateID: "wechat-template-meal-status-001",
	...
	attempt := engagementModel.SubscribeMessageAttempt{
		LogID: logRow.ID, Attempt: 1, Status: engagementModel.SubscribeLogFailed,
	...
	logs, err := service.ListSubscribeLogs(
		ctx,
		engagementRequest.SubscribeLogListQuery{
	...
	_, _, err = service.DeleteSubscribeTemplate(
		...
		created.ID,
		engagementRequest.SubscribeTemplateDeleteInput{
			Reason:          "验证已有发送记录禁止删除",
...
func TestSubscriptionAdminRejectsDirectInvalidTemplateAndLogInputs(t *testing.T) {
	...
	_, _, err = service.UpdateSubscribeTemplateStatus(
		...
		"template-id",
		engagementRequest.SubscribeTemplateStatusInput{
			Reason:          "非法启停输入",
			ExpectedVersion: 1,
		},
	)
	...
	_, err = service.ListSubscribeTemplates(
		context.Background(),
		engagementRequest.SubscribeTemplateListQuery{
