...
func pointEntryResponse(
	row engagementModel.FrontPointEntry,
	user userModel.MiniAppUser,
) engagementResponse.PointEntry {
	var administrator *commonResponse.AdministratorSummary
	...
	return engagementResponse.PointEntry{
		ID:                row.ID,
...
func (service *PointsService) ListPointEntries(
	ctx context.Context,
	query engagementRequest.PointEntryListQuery,
) (commonResponse.Page[engagementResponse.PointEntry], error) {
	query.ApplyDefaults()
	...
	if !ok {
		return commonResponse.Page[engagementResponse.PointEntry]{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	db := service.database()
	if db == nil {
		return commonResponse.Page[engagementResponse.PointEntry]{}, appErrors.AdminInternal.DefaultMsg()
	}
	statement := db.WithContext(ctx).Model(&engagementModel.FrontPointEntry{})
	...
	if err := statement.Count(&total).Error; err != nil {
		return commonResponse.Page[engagementResponse.PointEntry]{}, appErrors.AdminInternal.Wrap(err, "count point entries")
	}
	rows := make([]engagementModel.FrontPointEntry, 0)
	if err := statement.Order(column + " " + query.SortOrder).
		Order("id asc").
		Limit(query.PageSize).
		Offset((query.Page - 1) * query.PageSize).
		Find(&rows).Error; err != nil {
		return commonResponse.Page[engagementResponse.PointEntry]{}, appErrors.AdminInternal.Wrap(err, "list point entries")
	}
	userIDs := make([]string, 0, len(rows))
	for _, row := range rows {
		userIDs = append(userIDs, row.UserID)
	}
	users := make([]userModel.MiniAppUser, 0)
	if len(userIDs) > 0 {
		if err := db.WithContext(ctx).Where("id IN ?", userIDs).Find(&users).Error; err != nil {
			return commonResponse.Page[engagementResponse.PointEntry]{}, appErrors.AdminInternal.Wrap(err, "load point entry users")
		}
	}
	userMap := make(map[string]userModel.MiniAppUser, len(users))
	for _, user := range users {
		userMap[user.ID] = user
	}
	list := make([]engagementResponse.PointEntry, 0, len(rows))
	for _, row := range rows {
		list = append(list, pointEntryResponse(row, userMap[row.UserID]))
	}
	return commonResponse.Page[engagementResponse.PointEntry]{
...
func pointAdjustmentRequestHash(
	...
	input engagementRequest.PointAdjustmentPreviewInput,
) (string, error) {
	payload := struct {
	...
	return serviceCommon.HashIdempotencyPayload(payload)
}
...
func (service *PointsService) PreviewPointAdjustment(
	...
	input engagementRequest.PointAdjustmentPreviewInput,
) (engagementResponse.PointAdjustmentPreview, error) {
	if err := validatePointRuleActor(actor); err != nil {
		return engagementResponse.PointAdjustmentPreview{}, err
	}
	...
	if input.UserID == "" ||
		...
		return engagementResponse.PointAdjustmentPreview{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	db := service.database()
	if db == nil {
		return engagementResponse.PointAdjustmentPreview{}, appErrors.AdminInternal.DefaultMsg()
	}
	var user userModel.MiniAppUser
	if err := db.WithContext(ctx).First(&user, "id = ?", input.UserID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return engagementResponse.PointAdjustmentPreview{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return engagementResponse.PointAdjustmentPreview{}, appErrors.AdminInternal.Wrap(err, "load point adjustment user")
	}
	delta := input.Amount
	...
	if balanceAfter < 0 {
		return engagementResponse.PointAdjustmentPreview{}, appErrors.AdminNegativePoints.DefaultMsg()
	}
	...
	return engagementResponse.PointAdjustmentPreview{
		PreviewToken:        token,
		ExpiresAt:           expiresAt,
		User:                subscriptionUserReference(user),
		Direction:           input.Direction,
...
func (service *PointsService) CreatePointAdjustment(
...
) (engagementResponse.PointAdjustmentResult, bool, error) {
	if err := validatePointRuleActor(actor); err != nil {
		return engagementResponse.PointAdjustmentResult{}, false, err
	}
	...
	if input.ExpectedUserVersion != claims.ExpectedUserVersion {
		return engagementResponse.PointAdjustmentResult{}, false,
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			updateResult := tx.Model(&userModel.MiniAppUser{}).
			...
			entry := engagementModel.FrontPointEntry{
				ID:                    serviceCommon.NewPublicID("point"),
				UserID:                user.ID,
				Type:                  engagementModel.PointAdjustment,
				Scene:                 scene,
				Title:                 title,
			...
			result := engagementResponse.PointAdjustmentResult{
				UserID:        user.ID,
				BalanceBefore: balanceBefore,
				BalanceAfter:  balanceAfter,
	...
	return serviceCommon.DecodeIdempotentResult[engagementResponse.PointAdjustmentResult](raw, replayed, err)
}
...
func (service *PointsService) EnsureDefaults(ctx context.Context) error {
	...
	row := engagementModel.PointRuleConfig{
...
func latestPointRule(db *gorm.DB) (engagementModel.PointRuleConfig, error) {
	var row engagementModel.PointRuleConfig
	if err := db.Where("singleton_key = ?", pointRuleSingletonKey).First(&row).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return engagementModel.PointRuleConfig{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return engagementModel.PointRuleConfig{}, appErrors.AdminInternal.Wrap(err, "load current point rule")
	}
	return row, nil
}
...
func pointRuleConfigResponse(row engagementModel.PointRuleConfig) engagementResponse.PointRuleConfig {
	return engagementResponse.PointRuleConfig{
		Version:            row.Version,
...
func (service *PointsService) CurrentPointRule(ctx context.Context) (engagementResponse.PointRuleConfig, error) {
	if err := service.EnsureDefaults(ctx); err != nil {
		return engagementResponse.PointRuleConfig{}, err
	}
...
func validatePointRuleActor(actor commonRequest.AdminActor) error {
...
func (service *PointsService) UpdatePointRule(
	...
	actor commonRequest.AdminActor,
	idempotencyKey string,
	input engagementRequest.PointRuleUpdateInput,
) (engagementResponse.PointRuleConfig, bool, error) {
	if err := validatePointRuleActor(actor); err != nil {
		return engagementResponse.PointRuleConfig{}, false, err
	}
	reason := strings.TrimSpace(input.Reason)
	if input.DailyCheckinReward < 1 || input.DailyCheckinReward > 100 ||
		input.ExpectedVersion < 1 || len([]rune(reason)) < 2 || len([]rune(reason)) > 200 {
		return engagementResponse.PointRuleConfig{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	if err := service.EnsureDefaults(ctx); err != nil {
		return engagementResponse.PointRuleConfig{}, false, err
	}
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			next := engagementModel.PointRuleConfig{
				SingletonKey:       pointRuleSingletonKey,
			...
			update := tx.Model(&engagementModel.PointRuleConfig{}).
	...
	return serviceCommon.DecodeIdempotentResult[engagementResponse.PointRuleConfig](raw, replayed, err)
}
