...
func (catalogUserDishReference) TableName() string {
	return (contentModel.UserDish{}).TableName()
}
...
func (catalogStandardDishReference) TableName() string {
	return (dishModel.StandardDishIndex{}).TableName()
}
...
func (catalogOfficialDishReference) TableName() string {
	return (dishModel.OfficialDish{}).TableName()
}
...
func (catalogDishIngredientReference) TableName() string {
	return (contentModel.DishIngredient{}).TableName()
}
...
func (catalogOfficialIngredientReference) TableName() string {
	return (dishModel.OfficialDishIngredient{}).TableName()
}
...
func newCatalogTestService(t *testing.T) (*CatalogService, *gorm.DB) {
	...
	service := NewCatalogService(db, serviceCommon.NewIdempotencyService(db))
...
func TestCatalogListsAllResourceReferencesAndValidatesQuery(t *testing.T) {
	...
	category, _, err := service.CreateCategory(
		...
		dishRequest.CatalogItemCreateInput{Name: "热菜", SortOrder: 1, Enabled: &enabled},
	)
	if err != nil {
		t.Fatal(err)
	}
	tag, _, err := service.CreateTag(
		...
		dishRequest.CatalogItemCreateInput{Name: "下饭", SortOrder: 1, Enabled: &enabled},
	)
	if err != nil {
		t.Fatal(err)
	}
	unit, _, err := service.CreateUnit(
		...
		dishRequest.CatalogItemCreateInput{Name: "克", SortOrder: 1, Enabled: &enabled},
	)
	if err != nil {
		t.Fatal(err)
	}

	var categoryRow dishModel.ContentCategory
	var tagRow dishModel.ContentTag
	var unitRow dishModel.ContentUnit
	...
	references := []interface{}{
		...
		&contentModel.UserDishTag{DishID: 1, TagID: tagRow.ID, CreatedAt: service.now()},
		&dishModel.OfficialDishTag{DishID: 1, TagID: tagRow.ID, CreatedAt: service.now()},
	...
	categoryPage, err := service.ListCategories(ctx, dishRequest.CatalogItemListQuery{})
	if err != nil {
		t.Fatal(err)
	}
	tagPage, err := service.ListTags(ctx, dishRequest.CatalogItemListQuery{})
	if err != nil {
		t.Fatal(err)
	}
	unitPage, err := service.ListUnits(ctx, dishRequest.CatalogItemListQuery{})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = service.ListCategories(ctx, dishRequest.CatalogItemListQuery{
	...
	_, err = service.ListCategories(ctx, dishRequest.CatalogItemListQuery{PageSize: 21})
	if appErrors.GetType(err) != appErrors.AdminBadRequest {
...
func TestCatalogTagLifecycleAndReferenceProtection(t *testing.T) {
	...
	created, replayed, err := service.CreateTag(
		...
		actor,
		"catalog-tag-create",
		dishRequest.CatalogItemCreateInput{
	...
	updated, replayed, err := service.UpdateTag(
		...
		actor,
		"catalog-tag-update",
		dishRequest.CatalogItemUpdateInput{
	...
	sorted, replayed, err := service.UpdateTagSortOrder(
		...
		actor,
		"catalog-tag-sort",
		dishRequest.CatalogSortOrderInput{
			Items: []dishRequest.CatalogSortOrderItem{
	...
	var stored dishModel.ContentTag
	if err := db.First(&stored, "public_id = ?", created.ID).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&contentModel.UserDishTag{
	...
	_, _, err = service.DeleteTag(
		...
		actor,
		"catalog-tag-delete",
		dishRequest.CatalogDeleteInput{
	...
	if err := db.Where("tag_id = ?", stored.ID).
		Delete(&contentModel.UserDishTag{}).Error; err != nil {
		t.Fatal(err)
	}
