...
func weChatConfigTestService(t *testing.T) (*WeChatConfigService, *gorm.DB) {
	...
	service := NewWeChatConfigService(db, permission, serviceCommon.NewIdempotencyService(db))
...
func TestWeChatConfigServiceEncryptsSecretAndSupportsSafeUpdates(t *testing.T) {
	...
	first, replayed, err := service.UpdateConfig(
		...
		actor,
		"wechat-config-first",
		userRequest.WeChatConfigUpdateInput{
	...
	var persisted userModel.WeChatConfig
	...
	var updated userModel.WeChatConfig
