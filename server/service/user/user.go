...
type UserService struct {
	DB          *gorm.DB                          // 业务数据库
	Audit       *serviceCommon.AccessAuditService // 敏感数据访问审计服务
...
func NewUserService(
	db *gorm.DB,
	audit *serviceCommon.AccessAuditService,
...
func capabilityEffects(effective string) userResponse.CapabilityEffects {
	enabled := effective == "enabled"
	return userResponse.CapabilityEffects{
...
func activeCreatedMealImpact(
	...
	impact := &userResponse.UserActiveMealImpact{
		MealID: meal.ID,
		Status: string(meal.Status),
	}
	if err := db.Model(&mealModel.MealParticipant{}).
	...
	if err := db.Model(&mealModel.FrontMealFinalDish{}).
		Where("meal_id = ?", meal.ID).
...
func (service *UserService) List(
	ctx context.Context,
	query userRequest.UserListQuery,
) (commonResponse.Page[userResponse.UserSummary], error) {
	query.ApplyDefaults()
	db := service.database()
	if db == nil {
		return commonResponse.Page[userResponse.UserSummary]{}, appErrors.AdminInternal.DefaultMsg()
	}
	policy, err := service.policy(ctx)
	if err != nil {
		return commonResponse.Page[userResponse.UserSummary]{}, err
	}
	if query.MinPoints != nil && query.MaxPoints != nil && *query.MinPoints > *query.MaxPoints {
		return commonResponse.Page[userResponse.UserSummary]{}, appErrors.AdminBadRequest.DefaultMsg()
	}

	statement := db.WithContext(ctx).Model(&userModel.MiniAppUser{})
	...
	if activeFrom != "" || activeTo != "" {
		activity := db.WithContext(ctx).Model(&engagementModel.FrontUserActivityDay{}).
			Select("user_id")
		if activeFrom != "" {
	...
	if err := statement.Count(&total).Error; err != nil {
		return commonResponse.Page[userResponse.UserSummary]{}, appErrors.AdminInternal.Wrap(err, "count users")
	}
	...
	if err := statement.
		...
		return commonResponse.Page[userResponse.UserSummary]{}, appErrors.AdminInternal.Wrap(err, "list users")
	}
	list := make([]userResponse.UserSummary, 0, len(users))
	for index := range users {
		list = append(list, toUserSummary(users[index], policy))
	}
	return commonResponse.Page[userResponse.UserSummary]{
		Page:     query.Page,
...
func (service *UserService) Detail(
	ctx context.Context,
	userID string,
) (userResponse.UserDetail, error) {
	db := service.database()
	if db == nil || userID == "" {
		if userID == "" {
			return userResponse.UserDetail{}, appErrors.AdminBadRequest.DefaultMsg()
		}
		return userResponse.UserDetail{}, appErrors.AdminInternal.DefaultMsg()
	}
	policy, err := service.policy(ctx)
	if err != nil {
		return userResponse.UserDetail{}, err
	}
	var user userModel.MiniAppUser
	if err := db.WithContext(ctx).First(&user, "id = ?", userID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return userResponse.UserDetail{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return userResponse.UserDetail{}, appErrors.AdminInternal.Wrap(err, "get user")
	}
...
func toUserSummary(
	...
	return userResponse.UserSummary{
		ID:                  user.ID,
...
func cancelCreatorMealsForDisabledUser(
	...
	cancelledResult := string(mealModel.MealCancelled)
	for _, meal := range meals {
		var snapshotCount int64
		if err := tx.Model(&mealModel.FrontMealFinalDish{}).
			Where("meal_id = ?", meal.ID).
		...
		if err := tx.Model(&mealModel.FrontShoppingList{}).
			Where("meal_id = ?", meal.ID).
		...
		result := tx.Model(&mealModel.FrontMeal{}).
			Where("id = ? AND status = ?", meal.ID, meal.Status).
			Updates(map[string]interface{}{
				"status":                mealModel.MealCancelled,
...
func (service *UserService) UpdateStatus(
...
) (userResponse.UserStatusResult, bool, error) {
	if err := serviceCommon.ValidateReason(body.Reason); err != nil {
		return userResponse.UserStatusResult{}, false, err
	}
	...
	payload := struct {
		...
		Body   userRequest.UpdateUserStatusBody `json:"body"`
	...
	var result userResponse.UserStatusResult
	if err := json.Unmarshal(raw, &result); err != nil {
		return userResponse.UserStatusResult{}, false, appErrors.AdminInternal.Wrap(err, "decode status response")
	}
...
func (service *UserService) updateStatusWithDB(
	...
	idempotencyKey string,
	body userRequest.UpdateUserStatusBody,
) (userResponse.UserStatusResult, error) {
	var current userModel.MiniAppUser
	if err := tx.
		First(&current, "id = ?", userID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return userResponse.UserStatusResult{}, appErrors.AdminNotFound.DefaultMsg()
		}
		return userResponse.UserStatusResult{}, appErrors.AdminInternal.Wrap(err, "get user for status update")
	}
	if current.Version != body.ExpectedVersion || current.Status == body.Status {
		return userResponse.UserStatusResult{}, appErrors.AdminStateConflict.DefaultMsg()
	}
	now := service.now()
	updates := map[string]interface{}{
		"status":     body.Status,
	...
	if body.Status == userModel.UserStatusDisabled {
	...
	result := tx.Model(&userModel.MiniAppUser{}).
		Where("id = ? AND version = ?", userID, body.ExpectedVersion).
		Updates(updates)
	if result.Error != nil {
		return userResponse.UserStatusResult{}, appErrors.AdminInternal.Wrap(result.Error, "update user status")
	}
	if result.RowsAffected != 1 {
		return userResponse.UserStatusResult{}, appErrors.AdminStateConflict.DefaultMsg()
	}
	...
	if body.Status == userModel.UserStatusDisabled {
		...
		if mealErr != nil {
			return userResponse.UserStatusResult{}, mealErr
		}
	}
	var updated userModel.MiniAppUser
	if err := tx.First(&updated, "id = ?", userID).Error; err != nil {
		return userResponse.UserStatusResult{}, appErrors.AdminInternal.Wrap(err, "reload user status")
	}
	responseResult := userResponse.UserStatusResult{
		UserID:                     updated.ID,
	...
	if err := service.writeUserMutationAudit(
		...
		return userResponse.UserStatusResult{}, err
	}
...
func (service *UserService) UpdateCapability(
	...
	userID string,
	idempotencyKey string,
	body userRequest.UpdateUserCapabilityBody,
) (userResponse.UserCapabilityResult, bool, error) {
	if err := serviceCommon.ValidateReason(body.Reason); err != nil {
		return userResponse.UserCapabilityResult{}, false, err
	}
	if err := serviceCommon.ValidateAdminActor(actor); err != nil || strings.TrimSpace(userID) == "" {
		return userResponse.UserCapabilityResult{}, false, appErrors.AdminBadRequest.DefaultMsg()
	}
	payload := struct {
		UserID string                               `json:"userId"`
		Body   userRequest.UpdateUserCapabilityBody `json:"body"`
	}{UserID: userID, Body: body}
	raw, replayed, err := service.Idempotency.Execute(
		ctx,
		actor.AdministratorID,
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			responseResult := userResponse.UserCapabilityResult{
	...
	var result userResponse.UserCapabilityResult
	if err := json.Unmarshal(raw, &result); err != nil {
		return userResponse.UserCapabilityResult{}, false, appErrors.AdminInternal.Wrap(err, "decode capability response")
	}
	return result, replayed, nil
}
...
func (service *UserService) PreferenceProfile(
	ctx context.Context,
	userID string,
	access serviceCommon.SensitiveAccess,
) (userResponse.UserPreferenceProfile, error) {
	db := service.database()
	if db == nil {
		return userResponse.UserPreferenceProfile{}, appErrors.AdminInternal.DefaultMsg()
	}
	if userID == "" ||
		access.Permission != PermissionUserPreferenceRead ||
		access.AdministratorID == 0 {
		return userResponse.UserPreferenceProfile{}, appErrors.AdminBadRequest.DefaultMsg()
	}
	policy, err := service.policy(ctx)
	if err != nil {
		return userResponse.UserPreferenceProfile{}, err
	}

	var result userResponse.UserPreferenceProfile
	err = db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		var stored userModel.UserPreferenceProfile
		...
		var evidence []userModel.PreferenceEvidenceAggregate
...
func buildPreferenceProfile(
	user userModel.MiniAppUser,
	stored userModel.UserPreferenceProfile,
	found bool,
	policy CapabilityPolicySnapshot,
) (userResponse.UserPreferenceProfile, error) {
	effective, _ := capabilityState(policy, user.CapabilityDisabled)
	result := userResponse.UserPreferenceProfile{
		User: commonResponse.UserReference{
			ID:        user.ID,
			Nickname:  user.Nickname,
			AvatarURL: user.AvatarURL,
			Status:    string(user.Status),
		...
		UpdateState:     string(userModel.PreferenceUpdatePending),
		UpdateEnabled:   effective == "enabled",
		CheckinDayCount: user.CheckinDayCount,
		EvidenceSources: []userResponse.PreferenceEvidenceSourceSummary{},
	}
	if !found {
		if effective != "enabled" {
	...
	if stored.HasProfile && len(stored.ProfileJSON) > 0 {
		content := userResponse.PreferenceProfileContent{
			CommonDishTags:            []userResponse.PreferenceTermSummary{},
			FrequentlyOrderedDishTags: []userResponse.PreferenceTermSummary{},
			CommonIngredients:         []userResponse.PreferenceTermSummary{},
			UserSettings:              []userResponse.PreferenceUserSettingSummary{},
		}
		if err := json.Unmarshal(stored.ProfileJSON, &content); err != nil {
			return userResponse.UserPreferenceProfile{},
				appErrors.AdminInternal.Wrap(err, "decode preference profile")
		}
		result.Profile = &content
	}
	if len(stored.LatestFailureJSON) > 0 {
		var failure userResponse.PreferenceProfileFailureSummary
...
func setCapabilityPaused(result *userResponse.UserPreferenceProfile) {
	code := "capability_disabled"
	summary := "整体增强能力已关闭，历史画像保留但暂停更新"
	result.UpdateState = string(userModel.PreferenceUpdatePaused)
...
func buildEvidenceSummaries(
	aggregates []userModel.PreferenceEvidenceAggregate,
) []userResponse.PreferenceEvidenceSourceSummary {
	byType := make(map[string]userModel.PreferenceEvidenceAggregate, len(aggregates))
	for index := range aggregates {
		byType[aggregates[index].SourceType] = aggregates[index]
	}
	result := make([]userResponse.PreferenceEvidenceSourceSummary, 0, len(preferenceEvidenceSourceTypes))
	for _, sourceType := range preferenceEvidenceSourceTypes {
		aggregate := byType[sourceType]
		result = append(result, userResponse.PreferenceEvidenceSourceSummary{
