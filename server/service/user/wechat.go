...
type WeChatConfigService struct {
	DB            *gorm.DB                          // 业务数据库
	Permission    *serviceCommon.PermissionService  // GVA按钮权限服务
...
func validateWeChatActor(actor commonRequest.AdminActor) error {
...
func LoadCurrentWeChatCredentials(
	...
	var current userModel.WeChatConfig
...
func weChatConfigResponse(
	current userModel.WeChatConfig,
	exists bool,
) userResponse.WeChatConfig {
	if !exists {
		return userResponse.WeChatConfig{}
	}
	updatedAt := current.AppliedAt
	administrator := serviceCommon.AdministratorSummary(
		current.AppliedByID,
...
func (service *WeChatConfigService) GetConfig(
	ctx context.Context,
	actor commonRequest.AdminActor,
) (userResponse.WeChatConfig, error) {
	if err := validateWeChatActor(actor); err != nil {
		return userResponse.WeChatConfig{}, err
	}
	...
	db := service.database()
	if db == nil {
		return userResponse.WeChatConfig{}, appErrors.AdminInternal.DefaultMsg()
	}
	var current userModel.WeChatConfig
	if err := db.WithContext(ctx).
		Where("singleton_key = ?", wechatConfigSingletonKey).
		First(&current).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return userResponse.WeChatConfig{}, nil
		}
		return userResponse.WeChatConfig{}, appErrors.AdminInternal.Wrap(err, "get wechat configuration")
	}
...
func validateWeChatConfigInput(
	input userRequest.WeChatConfigUpdateInput,
	current userModel.WeChatConfig,
	exists bool,
) error {
...
func (service *WeChatConfigService) writeWeChatConfigAudit(
	...
	reason string,
	before userResponse.WeChatConfig,
	after userResponse.WeChatConfig,
) error {
...
func (service *WeChatConfigService) UpdateConfig(
	...
	idempotencyKey string,
	input userRequest.WeChatConfigUpdateInput,
) (userResponse.WeChatConfig, bool, error) {
	if err := validateWeChatActor(actor); err != nil {
		return userResponse.WeChatConfig{}, false, err
	}
	if err := service.require(ctx, actor, PermissionWeChatConfigUpdate); err != nil {
		return userResponse.WeChatConfig{}, false, err
	}
	if service == nil || service.Idempotency == nil {
		return userResponse.WeChatConfig{}, false, appErrors.AdminInternal.DefaultMsg()
	}
	...
	raw, replayed, err := service.Idempotency.Execute(
		...
		func(tx *gorm.DB) (interface{}, error) {
			...
			if exists {
				update := tx.Model(&userModel.WeChatConfig{}).
	...
	return serviceCommon.DecodeIdempotentResult[userResponse.WeChatConfig](raw, replayed, err)
}
