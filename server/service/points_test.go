...
func TestListPointEntriesFiltersAndReturnsUserMySQL(t *testing.T) {
	...
	entries := []orderfoodModel.FrontPointEntry{
		{
			ID: "point_list_target", UserID: users[0].ID,
			Type: orderfoodModel.PointSpent, Scene: "feature_usage",
	...
	result, err := service.ListPointEntries(
		context.Background(),
		orderfoodRequest.PointEntryListQuery{
			UserID: users[0].ID, Type: string(orderfoodModel.PointSpent),
...
func TestPointAdjustmentRejectsStaleOrForgedPreviewState(t *testing.T) {
	...
	users := []orderfoodModel.MiniAppUser{
		{
			...
			Points: 20, Status: orderfoodModel.UserStatusNormal, Version: 1,
			RegisteredAt: now, CreatedAt: now, UpdatedAt: now,
		},
		{
	...
	_, _, err = service.CreatePointAdjustment(
		...
		orderfoodRequest.PointAdjustmentCreateInput{
...
func TestPointRuleUpdateAppliesImmediatelyAndRuntimeResolution(t *testing.T) {
	...
	updated, replayed, err := service.UpdatePointRule(
		...
		actor,
		"update-point-rule",
		orderfoodRequest.PointRuleUpdateInput{
	...
	if err := db.Model(&orderfoodModel.PointRuleConfig{}).Count(&configCount).Error; err != nil {
		t.Fatal(err)
	}
	if configCount != 1 {
