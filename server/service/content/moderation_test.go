...
func openModerationTestDB(t *testing.T) *gorm.DB {
	...
	models := []interface{}{
		...
		&contentModel.ModerationConfig{},
		&contentModel.ModerationConnectionTest{},
		&contentModel.ModerationImageTest{},
		&contentModel.ModerationConfigHealth{},
...
func moderationTestActor() commonRequest.AdminActor {
...
func TestModerationConfigSaveAppliesImmediatelyAndTestsCurrentConfig(t *testing.T) {
	...
	updated, replayed, err := service.UpdateConfig(
		ctx,
		contentRequest.ModerationConfigUpdateInput{
			Enabled:         true,
			Region:          "cn-shanghai",
	...
	if err := db.Model(&contentModel.ModerationConfig{}).Count(&configCount).Error; err != nil {
		t.Fatal(err)
	}
	if configCount != 1 {
	...
	connection, _, err := service.TestConnection(
		ctx,
		contentRequest.ModerationConfigTestInput{ExpectedVersion: &expectedVersion},
		actor,
	...
	if err != nil || imageResult.MappedStatus != string(contentModel.ModerationStatusPassed) ||
...
func TestImageModerationGatePersistsOnlyExplicitPass(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			name: "content risk",
			result: ModerationProviderResult{
				Status:      contentModel.ModerationStatusRejected,
...
func TestDisabledModerationAllowsUserImage(t *testing.T) {
	...
	persisted, decision, err := service.AcceptUserImage(
		...
		ModerationGateInput{
			...
			Scene:         contentModel.ModerationSceneProfileAvatar,
			ContentType:   "image/png",
			Bytes:         moderationTinyPNG,
		},
		func(tx *gorm.DB, permit PassedImagePermit) (interface{}, error) {
			writerCalls++
			if permit.ReviewStatus != contentModel.ModerationStatusNotRequired {
				t.Fatalf("review status = %q, want %q", permit.ReviewStatus, contentModel.ModerationStatusNotRequired)
			}
			row := moderationAcceptedBusinessRow{
				StagingFileID:      "disabled-moderation-file",
				ModerationRecordID: permit.ModerationRecordID,
	...
	if provider.imageCalls != 0 || !decision.Allowed ||
		decision.Status != string(contentModel.ModerationStatusNotRequired) {
...
func TestGeneratedCoverBypassesUploadModeration(t *testing.T) {
	...
	persisted, decision, err := service.AcceptUserImage(
		...
		ModerationGateInput{
			...
			Scene:         contentModel.ModerationSceneGeneratedCover,
			ContentType:   "image/png",
			Bytes:         moderationTinyPNG,
		},
		func(tx *gorm.DB, permit PassedImagePermit) (interface{}, error) {
