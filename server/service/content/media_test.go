...
func TestModerationRecordSummaryIncludesUserObjectAndProviderRequestMySQL(t *testing.T) {
	...
	asset := contentModel.FrontMediaAsset{
		...
		UploadSource: "user_upload", Scene: string(contentModel.ModerationSceneDishCover),
		...
		ReviewStatus: string(contentModel.ModerationStatusPassed), CreatedAt: now,
	}
	...
	list, err := service.ListModerationRecords(
		context.Background(),
		contentRequest.ModerationRecordListQuery{UserID: user.ID, ObjectType: objectType},
	)
	...
	rejectedRecord.Status = contentModel.ModerationStatusRejected
	failedRecord := record
	failedRecord.ID = "moderation-record-failed"
	failedRecord.RequestID = "request-id-failed"
	failedRecord.Status = contentModel.ModerationStatusFailed
	if err := db.Create(&[]contentModel.ImageModerationRecord{
		rejectedRecord,
		failedRecord,
	}).Error; err != nil {
	...
	abnormalList, err := service.ListModerationRecords(
		context.Background(),
		contentRequest.ModerationRecordListQuery{Status: "rejected_or_failed"},
	)
	...
	_, err = service.ListModerationRecords(
		context.Background(),
		contentRequest.ModerationRecordListQuery{
...
func TestMediaSummaryPreservesDeletedCheckinBindingMySQL(t *testing.T) {
	...
	asset := contentModel.FrontMediaAsset{
		...
		UploadSource: "user_upload", Scene: string(contentModel.ModerationSceneCheckin),
	...
	_, err = service.ListMedia(
		context.Background(),
		contentRequest.MediaListQuery{SortOrder: "desc; drop table of_media"},
	)
	if appErrors.GetType(err) != appErrors.AdminBadRequest {
	...
	officialAsset := contentModel.FrontMediaAsset{
		ID: "official-media-file", FileName: "official.png",
		UploadSource:   "admin_upload",
		Scene:          string(contentModel.ModerationSceneOfficialCover),
		...
		ReviewStatus: string(contentModel.ModerationStatusNotRequired), CreatedAt: now,
	}
	if err := db.Create(&officialAsset).Error; err != nil {
		t.Fatalf("create official media asset: %v", err)
	}
	officialDish := dishModel.OfficialDish{
