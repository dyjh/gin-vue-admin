...
type GovernanceNotificationWriter interface {
	WriteGovernanceNotification(context.Context, *gorm.DB, *engagementModel.UserNotification) (string, error)
}
...
func WithMutationAuditWriter(writer serviceCommon.MutationAuditWriter) ContentServiceOption {
...
func (gormGovernanceNotificationWriter) WriteGovernanceNotification(
	...
	if notification.ID == "" {
		notification.ID = serviceCommon.NewPublicID("notice")
	}
...
type ContentService struct {
	...
	mutationAudit serviceCommon.MutationAuditWriter
...
func (s *ContentService) database() *gorm.DB {
	if s != nil && s.DB != nil {
		return s.DB
	}
...
func (s *ContentService) ListUserDishes(
	ctx context.Context,
	search contentRequest.UserDishSearch,
	actor commonRequest.AdminActor,
) (commonResponse.Page[contentResponse.UserDishAdminSummary], error) {
	var result commonResponse.Page[contentResponse.UserDishAdminSummary]
	if s.database() == nil || s.permission == nil {
	...
	if !serviceCommon.ValidTimeRange(search.CreatedFrom, search.CreatedTo) ||
		!serviceCommon.ValidTimeRange(search.UpdatedFrom, search.UpdatedTo) ||
		!serviceCommon.ValidTimeRange(search.DeletedFrom, search.DeletedTo) ||
		!serviceCommon.ValidPagination(search.Page, search.PageSize) ||
	...
	query := s.database().WithContext(ctx).Model(&contentModel.UserDish{})
	...
	if value := strings.TrimSpace(search.UserKeyword); value != "" {
		keyword := "%" + value + "%"
		ownerIDs := s.database().Model(&userModel.MiniAppUser{}).
			Select("id").
	...
	if search.CategoryID != "" {
		query = query.Where(
			...
			s.database().Model(&dishModel.ContentCategory{}).Select("id").Where("public_id = ?", search.CategoryID),
		)
	}
	if len(search.TagIDs) > 0 {
		tagDishIDs := s.database().Table("of_dish_tags").
	...
	if search.RecipeID != "" {
		referencedDishIDs := s.database().Table("of_recipe_dishes").
	...
	result.List = make([]contentResponse.UserDishAdminSummary, 0, len(dishes))
	for i := range dishes {
		summary, summaryErr := s.buildDishSummary(ctx, s.database(), &dishes[i], includePrivateCover)
		if summaryErr != nil {
			return commonResponse.Page[contentResponse.UserDishAdminSummary]{}, summaryErr
		}
...
func (s *ContentService) GetUserDishDetail(
	...
	dishID string,
	actor commonRequest.AdminActor,
) (contentResponse.UserDishAdminDetail, error) {
	var detail contentResponse.UserDishAdminDetail
	if s.database() == nil || s.permission == nil || s.accessAudit == nil {
	...
	err = s.database().WithContext(ctx).Unscoped().
		Preload("Owner").
		Preload("Category").
		Preload("Tags").
	...
	summary, err := s.buildDishSummary(ctx, s.database(), &dish, true)
	if err != nil {
		return detail, err
	}
	sourceOverview, err := s.buildDishSourceOverview(ctx, s.database(), &dish)
	if err != nil {
		return detail, err
	}
	referenceOverview, err := s.buildDishReferenceOverview(ctx, s.database(), dish.ID)
	if err != nil {
		return detail, err
	}
	...
	detail = contentResponse.UserDishAdminDetail{
	...
	for _, ingredient := range dish.Ingredients {
		...
		detail.Ingredients = append(detail.Ingredients, contentResponse.DishIngredient{
	...
	for _, step := range dish.Steps {
		detail.Steps = append(detail.Steps, contentResponse.DishStep{
			Description: step.Description, ImageURL: step.ImageURL, SortOrder: step.SortOrder,
		})
	}
	var governanceCount int64
	if err := s.database().WithContext(ctx).Model(&contentModel.GovernanceRecord{}).
		Where("target_type = ? AND target_id = ?", contentModel.GovernanceTargetDish, dish.PublicID).
		Count(&governanceCount).Error; err != nil {
		return contentResponse.UserDishAdminDetail{}, appErrors.AdminInternal.Wrap(err, "count dish governance records")
	}
	detail.GovernanceRecordCount = int(governanceCount)
	var auditCount int64
	if err := s.database().WithContext(ctx).Model(&auditModel.AdminAuditLog{}).
		Where("target_type = ? AND target_id = ?", contentModel.GovernanceTargetDish, dish.PublicID).
		Count(&auditCount).Error; err != nil {
		return contentResponse.UserDishAdminDetail{}, appErrors.AdminInternal.Wrap(err, "count dish audit logs")
	}
...
func (s *ContentService) latestDishModerationSummary(
	...
	if s == nil || s.database() == nil || dish == nil {
		return nil, appErrors.AdminInternal.DefaultMsg()
	}
	statement := s.database().WithContext(ctx).Model(&contentModel.ImageModerationRecord{})
	...
	summary, err := ModerationRecordSummary(ctx, s.database(), record)
	if err != nil {
		return nil, err
	}
	return &summary, nil
...
func (s *ContentService) ListUserDishReferences(
	ctx context.Context,
	dishID string,
	search contentRequest.UserDishReferenceSearch,
	actor commonRequest.AdminActor,
) (commonResponse.AuditedPage[contentResponse.UserDishReference], error) {
	var result commonResponse.AuditedPage[contentResponse.UserDishReference]
	if s.database() == nil || s.permission == nil || s.accessAudit == nil {
		return result, appErrors.AdminInternal.DefaultMsg()
	}
	if strings.TrimSpace(dishID) == "" {
	...
	dishID = strings.TrimSpace(dishID)
	if !serviceCommon.ValidPagination(search.Page, search.PageSize) ||
	...
	page, pageSize := normalizePage(search.Page, search.PageSize)
	var dish contentModel.UserDish
	if err := s.database().WithContext(ctx).Unscoped().Select("id").Where("public_id = ?", dishID).First(&dish).Error; err != nil {
		if stderrors.Is(err, gorm.ErrRecordNotFound) {
			return result, appErrors.AdminNotFound.DefaultMsg()
		}
		return result, appErrors.AdminInternal.Wrap(err, "find referenced dish")
	}
	query := s.database().WithContext(ctx).Model(&contentModel.DishReference{}).Where("dish_id = ?", dish.ID)
	...
	result.List = make([]contentResponse.UserDishReference, 0, len(references))
	for _, reference := range references {
		result.List = append(result.List, contentResponse.UserDishReference{
...
func (s *ContentService) ListUserRecipes(
	ctx context.Context,
	search contentRequest.UserRecipeSearch,
	actor commonRequest.AdminActor,
) (commonResponse.Page[contentResponse.UserRecipeAdminSummary], error) {
	var result commonResponse.Page[contentResponse.UserRecipeAdminSummary]
	if s.database() == nil || s.permission == nil {
		return result, appErrors.AdminInternal.DefaultMsg()
	}
	...
	page, pageSize := normalizePage(search.Page, search.PageSize)
	query := s.database().WithContext(ctx).Model(&contentModel.UserRecipe{})
	if search.DeletionStatus == "deleted" {
	...
	if search.DishID != "" {
		recipeIDs := s.database().Table("of_recipe_dishes").
	...
	summaries := make([]contentResponse.UserRecipeAdminSummary, 0, len(recipes))
	for i := range recipes {
...
func (s *ContentService) GetUserRecipeDetail(
	...
	recipeID string,
	actor commonRequest.AdminActor,
) (contentResponse.UserRecipeAdminDetail, error) {
	var detail contentResponse.UserRecipeAdminDetail
	if s.database() == nil || s.permission == nil || s.accessAudit == nil {
		return detail, appErrors.AdminInternal.DefaultMsg()
	}
	...
	detail = contentResponse.UserRecipeAdminDetail{
	...
	if err := s.database().WithContext(ctx).Model(&contentModel.GovernanceRecord{}).
		...
		Count(&governanceCount).Error; err != nil {
		return contentResponse.UserRecipeAdminDetail{}, appErrors.AdminInternal.Wrap(err, "count recipe governance records")
	}
	detail.GovernanceRecordCount = int(governanceCount)
	var auditCount int64
	if err := s.database().WithContext(ctx).Model(&auditModel.AdminAuditLog{}).
		Where("target_type = ? AND target_id = ?", contentModel.GovernanceTargetRecipe, recipe.PublicID).
		Count(&auditCount).Error; err != nil {
		return contentResponse.UserRecipeAdminDetail{}, appErrors.AdminInternal.Wrap(err, "count recipe audit logs")
	}
	detail.AuditLogCount = int(auditCount)
	return detail, nil
}
...
func (s *ContentService) PreviewGovernance(
	...
	actor commonRequest.AdminActor,
) (contentResponse.GovernanceImpactPreview, error) {
	var preview contentResponse.GovernanceImpactPreview
	if s.database() == nil || s.permission == nil || len(s.effectivePreviewSecret()) == 0 {
	...
	impact, err := s.buildGovernanceImpact(ctx, s.database(), input)
	if err != nil {
		return preview, err
	}
...
func (s *ContentService) ExecuteGovernance(
	ctx context.Context,
	input contentRequest.GovernanceActionExecuteInput,
	actor commonRequest.AdminActor,
	idempotencyKey string,
) (contentResponse.GovernanceExecutionResult, error) {
	var result contentResponse.GovernanceExecutionResult
	if s.database() == nil || s.idempotency == nil || s.permission == nil ||
...
func (s *ContentService) executeGovernanceOnce(
	...
	case contentModel.GovernanceTargetCheckin:
		...
		if err := tx.Model(&engagementModel.FrontCheckin{}).
	...
	if recipientUserID != "" {
		...
		notificationID, err := s.notification.WriteGovernanceNotification(ctx, tx, &engagementModel.UserNotification{
	...
	record := contentModel.GovernanceRecord{
		PublicID:              serviceCommon.NewPublicID("gov"),
	...
	result = contentResponse.GovernanceExecutionResult{
...
func offlineGovernedSourceRecommendations(
	...
	statement := tx.WithContext(ctx).Model(&dishModel.PlatformRecommendation{}).
	...
	offlineReason := serviceCommon.TruncateRunes("来源菜品被平台处理："+strings.TrimSpace(reason), 200)
...
func removeGovernedDishLiveReferences(ctx context.Context, tx *gorm.DB, dishID uint) error {
	...
	if err := tx.WithContext(ctx).Model(&mealModel.FrontMealCandidate{}).
		Where(
			"dish_id = ? AND meal_id IN (?)",
			dishID,
			tx.Model(&mealModel.FrontMeal{}).Select("id").
				Where("status IN ?", []mealModel.MealStatus{
					mealModel.MealCollecting,
					mealModel.MealClosed,
				}),
		).
		Updates(map[string]interface{}{
...
func (s *ContentService) createCopyChainGovernanceJob(
	...
	actor commonRequest.AdminActor,
	idempotencyKey string,
	impact contentResponse.GovernanceImpactPreview,
) (contentResponse.GovernanceExecutionResult, error) {
	var result contentResponse.GovernanceExecutionResult
	if tx == nil {
		return result, appErrors.AdminInternal.DefaultMsg()
	...
	now := s.now().UTC()
	recordID := serviceCommon.NewPublicID("gov")
	jobID := serviceCommon.NewPublicID("govjob")
	...
	for index := range targets {
		items = append(items, contentModel.GovernanceJobItem{
			ID:           serviceCommon.NewPublicID("govitem"),
			JobID:        jobID,
	...
	return contentResponse.GovernanceExecutionResult{
		RecordID:      recordID,
		JobID:         &jobID,
		Status:        contentModel.GovernanceJobPending,
...
func (s *ContentService) createOfficialCopyChainGovernanceJob(
	...
	var source dishModel.OfficialDish
	if err := tx.WithContext(ctx).Unscoped().
		Where("public_id = ?", input.TargetID).
		First(&source).Error; err != nil {
		return result, mapTargetLookupError(err)
...
func (s *ContentService) buildDishSummary(
...
) (contentResponse.UserDishAdminSummary, error) {
	...
	if err != nil {
		return contentResponse.UserDishAdminSummary{}, appErrors.AdminInternal.Wrap(err, "count recipe references")
	}
	...
	if err := db.WithContext(ctx).Model(&contentModel.DishReference{}).
		...
		return contentResponse.UserDishAdminSummary{}, appErrors.AdminInternal.Wrap(err, "count meal references")
	}
	tags := make([]commonResponse.CatalogReference, 0, len(dish.Tags))
	for _, tag := range dish.Tags {
		tags = append(tags, commonResponse.CatalogReference{ID: tag.PublicID, Name: tag.Name, Enabled: tag.Enabled})
	}
	deletionStatus := "active"
	if dish.DeletedAt.Valid {
		deletionStatus = "deleted"
	}
	return contentResponse.UserDishAdminSummary{
		ID:       dish.PublicID,
		CoverURL: coverURL,
		Name:     dish.Name,
		Owner:    serviceCommon.UserReference(dish.Owner),
...
func (s *ContentService) buildDishReferenceOverview(
	...
	db *gorm.DB,
	dishID uint,
) (contentResponse.UserDishReferenceOverview, error) {
	var overview contentResponse.UserDishReferenceOverview
...
func (s *ContentService) buildDishSourceOverview(
	...
	db *gorm.DB,
	dish *contentModel.UserDish,
) (contentResponse.UserDishSourceOverview, error) {
	overview := contentResponse.UserDishSourceOverview{
	...
	if dish.OriginalAuthorID != nil {
		var author userModel.MiniAppUser
		if err := db.WithContext(ctx).Where("id = ?", *dish.OriginalAuthorID).First(&author).Error; err != nil {
			...
			reference := serviceCommon.UserReference(author)
	...
	case contentModel.GovernanceTargetDish:
		...
		for _, action := range input.Actions {
			...
			case contentModel.GovernanceActionDeleteCopyChain:
				...
				if err := db.WithContext(ctx).Model(&dishModel.PlatformRecommendation{}).
					Where("status = ?", serviceCommon.RecommendationStatusPublished).
					Where(
...
func (s *ContentService) sensitiveAccess(
	...
	return serviceCommon.SensitiveAccess{
		AdministratorID:       actor.AdministratorID,
		AdministratorUsername: actor.Username,
		AdministratorNickname: actor.Nickname,
...
func governanceRequestHash(input contentRequest.GovernanceActionPreviewInput) string {
...
func validateGovernanceInput(input contentRequest.GovernanceActionPreviewInput) error {
...
func buildRecipeSummary(recipe *contentModel.UserRecipe, includePrivateCover bool) contentResponse.UserRecipeAdminSummary {
	...
	return contentResponse.UserRecipeAdminSummary{
		ID: recipe.PublicID, Name: recipe.Name, CoverURL: coverURL,
		Owner: serviceCommon.UserReference(recipe.Owner), DishCount: len(dishItems), UnavailableDishCount: unavailableCount,
...
func buildRecipeDishItems(recipe *contentModel.UserRecipe) []contentResponse.UserRecipeDishItem {
	...
	items := make([]contentResponse.UserRecipeDishItem, 0, len(relations))
	for _, relation := range relations {
		...
		items = append(items, contentResponse.UserRecipeDishItem{
...
func loadDishSourceNode(ctx context.Context, db *gorm.DB, dishID uint) (*contentResponse.UserDishSourceNode, error) {
	var dish contentModel.UserDish
	...
	return &contentResponse.UserDishSourceNode{
...
func loadOfficialDishSourceNode(
	...
	db *gorm.DB,
	dishID uint,
) (*contentResponse.UserDishSourceNode, error) {
	var dish dishModel.OfficialDish
...
func deletedByAdministrator(
	...
	return &commonResponse.AdministratorSummary{ID: strconv.FormatUint(uint64(*id), 10), Username: value, Nickname: nickname}
}
...
func recipeSummaryLess(left, right contentResponse.UserRecipeAdminSummary, sortBy string) bool {
	switch sortBy {
	case "updatedAt":
...
func recipeSummaryEqual(left, right contentResponse.UserRecipeAdminSummary, sortBy string) bool {
	switch sortBy {
	case "updatedAt":
