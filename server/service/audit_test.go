...
func TestAuditServiceListAndDetailUseOperationAudit(t *testing.T) {
	...
	if err := db.AutoMigrate(&orderfoodModel.AdminAuditLog{}); err != nil {
	...
	record := orderfoodModel.AdminAuditLog{
		PublicID:              "audit-001",
		AdministratorID:       11,
	...
	page, err := service.List(context.Background(), orderfoodRequest.AuditLogListQuery{
		AdministratorID: "11",
		TargetType:      "user",
		RequestID:       "request-audit-001",
	})
...
func TestMaskIP(t *testing.T) {
	...
	for input, want := range tests {
		if got := MaskIP(input); got != want {
			t.Fatalf("MaskIP(%q) = %q, want %q", input, got, want)
		}
	}
}
