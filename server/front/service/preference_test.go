...
func TestPreferencePipelineCoversSevenSourcesAndKeepsPendingState(t *testing.T) {
	...
	cases := []evidenceCase{
		{
			sourceType: orderfoodModel.PreferenceSourceCheckinImage,
			sourceID:   "checkin-1",
			direction:  "positive",
			weight:     1.5,
		...
		{
			sourceType: orderfoodModel.PreferenceSourceRecommendationAdopted,
		...
		{
			sourceType: orderfoodModel.PreferenceSourceDishSaved,
			sourceID:   "dish-1",
			direction:  "positive",
			weight:     2.5,
		...
		{
			sourceType: orderfoodModel.PreferenceSourceDishOrdered,
		...
		{
			sourceType: orderfoodModel.PreferenceSourceExplicitSetting,
			sourceID:   "setting-1",
			direction:  "positive",
			weight:     4,
	...
	if err := db.Model(&orderfoodModel.PreferenceEvidenceAggregate{}).
	...
	if err := db.Where("user_id = ? AND source_type <> ?",
		userID, orderfoodModel.PreferenceSourceCheckinImage).
	...
	if !profile.HasProfile || profile.UpdateState != orderfoodModel.PreferenceUpdatePending {
		t.Fatalf("preference profile state = %+v", profile)
	}
	var content orderfoodResponse.PreferenceProfileContent
	...
	var pendingCheckin orderfoodModel.PreferenceEvidence
	if err := db.First(&pendingCheckin,
		"user_id = ? AND source_type = ?", userID, orderfoodModel.PreferenceSourceCheckinImage).Error; err != nil {
		t.Fatalf("load pending checkin evidence: %v", err)
	}
	if pendingCheckin.Status != orderfoodModel.PreferenceEvidencePending {
		t.Fatalf("checkin evidence status = %q, want %q",
			pendingCheckin.Status, orderfoodModel.PreferenceEvidencePending)
	}
}
