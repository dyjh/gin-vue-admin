...
func decodeMealSubscriptionPayload(raw datatypes.JSON) (map[string]string, error) {
	...
	case string(mealModel.MealConfirmed):
		payload["result"] = "菜单已确认"
	case string(mealModel.MealCancelled):
...
func subscriptionTemplateData(
	template engagementModel.SubscribeMessageTemplate,
...
func (service *SubscriptionDeliveryService) deliverLog(
	...
	return service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		claim := tx.Model(&engagementModel.SubscribeMessageLog{}).
			Where("id = ? AND status = ?", logID, engagementModel.SubscribeLogPending).
			Update("status", engagementModel.SubscribeLogSending)
		if claim.Error != nil {
		...
		if err := tx.Model(&engagementModel.SubscribeMessageAttempt{}).
		...
		if attemptCount >= subscribeMaxSendAttempts {
			return tx.Model(&row).Update("status", engagementModel.SubscribeLogFailed).Error
		}
		...
		attemptStatus := engagementModel.SubscribeLogSent
		if sendErr != nil {
			attemptStatus = engagementModel.SubscribeLogFailed
		}
		...
		if err := tx.Create(&engagementModel.SubscribeMessageAttempt{
		...
		if sendErr == nil {
			if err := tx.Model(&row).Updates(map[string]interface{}{
				"status": engagementModel.SubscribeLogSent, "retry_count": retryCount,
		...
		status := engagementModel.SubscribeLogPending
		...
		if err := tx.Model(&row).Updates(map[string]interface{}{
			...
			"authorization_available": status != engagementModel.SubscribeLogFailed,
		}).Error; err != nil {
			return appErrors.FrontInternal.Wrap(err, "record subscription send failure")
		}
		if status == engagementModel.SubscribeLogFailed {
			if err := tx.Model(&template).
...
func (service *SubscriptionDeliveryService) ProcessPendingMessages(
	...
	var logIDs []string
	if err := service.database().WithContext(ctx).Model(&engagementModel.SubscribeMessageLog{}).
		Where("status = ?", engagementModel.SubscribeLogPending).
