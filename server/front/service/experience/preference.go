...
func refreshEvidenceAggregateTx(
	...
	if err := tx.Model(&userModel.PreferenceEvidence{}).
	...
	row := userModel.PreferenceEvidenceAggregate{
...
func (service *PreferenceService) queueEvidenceTx(
	...
	row := userModel.PreferenceEvidence{
		ID: commonModel.NewID(), UserID: userID,
		SourceType: sourceType, SourceID: strings.TrimSpace(sourceID),
		Direction: direction, Confidence: confidence, Weight: weight,
		FactsJSON: datatypes.JSON(encoded), Status: userModel.PreferenceEvidencePending,
	...
	profile := userModel.UserPreferenceProfile{
		ID: commonModel.NewID(), UserID: userID,
		UpdateState: userModel.PreferenceUpdatePending, UpdateEnabled: true,
		LastEvidenceAt: &occurredAt, Version: 1, CreatedAt: now, UpdatedAt: now,
	}
	if err := tx.Clauses(clause.OnConflict{
		Columns: []clause.Column{{Name: "user_id"}},
		DoUpdates: clause.Assignments(map[string]interface{}{
			"update_state":   userModel.PreferenceUpdatePending,
...
func preferenceTermSummaries(values []string) []userResponse.PreferenceTermSummary {
	result := make([]userResponse.PreferenceTermSummary, 0, len(values))
	for _, value := range values {
		result = append(result, userResponse.PreferenceTermSummary{
...
func (service *PreferenceService) summarizePreferenceProfile(
	...
	var stored []userModel.PreferenceEvidence
	if err := service.database().WithContext(ctx).
		Where("user_id = ? AND status = ?", row.UserID, userModel.PreferenceEvidenceAggregated).
...
func (service *PreferenceService) rebuildPreferenceProfileTx(
	...
	if err := tx.Where("user_id = ? AND status = ?",
		userID, userModel.PreferenceEvidenceAggregated).
	...
	for _, row := range rows {
		...
		if row.SourceType == userModel.PreferenceSourceDishOrdered {
			addPreferenceScores(orderedTagScores, facts.Tags, score)
		}
		if row.SourceType == userModel.PreferenceSourceExplicitSetting {
			for _, setting := range facts.Settings {
	...
	settings := make([]userResponse.PreferenceUserSettingSummary, 0, len(settingsByKey))
	settingTexts := make([]string, 0, len(settingsByKey))
	for _, setting := range settingsByKey {
		settings = append(settings, userResponse.PreferenceUserSettingSummary{
	...
	if summary.TastePreferenceSummary != "" {
		content.TastePreferenceSummary = &userResponse.PreferenceTextSummary{
	...
	if len(settingTexts) > 0 {
		content.AvoidanceOrPreferenceSummary = &userResponse.PreferenceTextSummary{
	...
	var pendingCount int64
	if err := tx.Model(&userModel.PreferenceEvidence{}).
		Where("user_id = ? AND status = ?", userID, userModel.PreferenceEvidencePending).
		Count(&pendingCount).Error; err != nil {
	...
	if pendingCount > 0 {
		updateState = userModel.PreferenceUpdatePending
	}
	if err := tx.Model(&userModel.UserPreferenceProfile{}).
...
func (service *PreferenceService) markPreferenceEvidenceFailure(
	...
	return service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var current userModel.PreferenceEvidence
		if err := tx.
			First(&current, "id = ?", row.ID).Error; err != nil {
			return appErrors.FrontInternal.Wrap(err, "load failed preference evidence")
		}
		if current.Status != userModel.PreferenceEvidencePending {
			return nil
		}
		retryCount := current.RetryCount + 1
		status := userModel.PreferenceEvidencePending
		if retryCount >= preferenceEvidenceMaxAttempts {
			status = userModel.PreferenceEvidenceFailed
		}
		summary := "偏好证据处理失败，系统将自动重试"
		if status == userModel.PreferenceEvidenceFailed {
			summary = "偏好证据多次处理失败"
		}
		...
		if status == userModel.PreferenceEvidenceFailed {
			failure, err := json.Marshal(userResponse.PreferenceProfileFailureSummary{
			...
			if err := tx.Model(&userModel.UserPreferenceProfile{}).
				...
				Updates(map[string]interface{}{
					"update_state":        userModel.PreferenceUpdateFailed,
...
func (service *PreferenceService) processPreferenceEvidence(
	...
	if row.SourceType == userModel.PreferenceSourceCheckinImage {
	...
	return service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&current).Updates(map[string]interface{}{
			...
			"status":          userModel.PreferenceEvidenceAggregated,
...
func (service *PreferenceService) ProcessPendingEvidence(
	...
	var rows []userModel.PreferenceEvidence
	eligibleUsers := service.database().Model(&userModel.MiniAppUser{}).
		Select("id").Where("capability_disabled = ?", false)
	if err := service.database().WithContext(ctx).
		Where("status = ? AND user_id IN (?)", userModel.PreferenceEvidencePending, eligibleUsers).
