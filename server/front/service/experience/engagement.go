...
func checkinResponse(row engagementModel.FrontCheckin) frontResponse.Checkin {
...
func (service *EngagementService) CreateCheckin(
	...
	var result engagementModel.FrontCheckin
	rewarded := false
	...
	err = db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.First(&asset,
			...
			input.ImageFileID, userID, "checkin", contentModel.UsableMediaReviewStatuses(),
		).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		result = engagementModel.FrontCheckin{
			ID: commonModel.NewID(), UserID: userID, DishName: input.DishName,
		...
		if rewarded {
			...
			entry := engagementModel.FrontPointEntry{
				ID: commonModel.NewID(), UserID: userID, Type: engagementModel.PointEarned,
				Scene: scene,
		...
		userUpdate := tx.Model(&userModel.MiniAppUser{}).
		...
		if analysisQueued {
			...
			if err := ServiceGroupApp.PreferenceService.queueEvidenceTx(
				ctx, tx, userID,
				userModel.PreferenceSourceCheckinImage, result.ID,
				"positive", 1.5, 0.6,
...
func (service *EngagementService) CheckinCalendar(
	...
	var dates []string
	if err := service.database().WithContext(ctx).Model(&engagementModel.FrontCheckin{}).
...
func (service *EngagementService) Checkins(
	...
	var total int64
	if err := db.Model(&engagementModel.FrontCheckin{}).Where("user_id = ?", userID).Count(&total).Error; err != nil {
		return frontResponse.Page[frontResponse.Checkin]{}, appErrors.FrontInternal.Wrap(err, "count checkins")
	}
	var rows []engagementModel.FrontCheckin
...
func (service *EngagementService) PointsSummary(
	...
	var earned int64
	var spent int64
	if err := service.database().WithContext(ctx).Model(&engagementModel.FrontPointEntry{}).
...
func (service *EngagementService) PointEntries(
	...
	statement := service.database().WithContext(ctx).Model(&engagementModel.FrontPointEntry{}).
	...
	var rows []engagementModel.FrontPointEntry
...
func (service *EngagementService) FeatureUsages(
	...
	query.PageQuery.Defaults()
	statement := service.database().WithContext(ctx).Model(&aiModel.FrontFeatureUsage{}).
		Where("user_id = ?", userID)
	if query.Feature != "" {
	...
	var rows []aiModel.FrontFeatureUsage
	if err := statement.Order("created_at desc, id desc").
		Limit(query.PageSize).Offset((query.Page - 1) * query.PageSize).Find(&rows).Error; err != nil {
	...
	for _, row := range rows {
		...
		var refundEntry engagementModel.FrontPointEntry
		if err := service.database().WithContext(ctx).
			Where(
				"type = ? AND related_object_type = ? AND related_object_id = ?",
				engagementModel.PointRefund, "feature_usage", row.ID,
			).
...
func (service *EngagementService) BeginFeatureUsage(
	...
	err = service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		activeExecutionStatuses := []string{
			aiModel.FeatureExecutionProcessing,
			aiModel.FeatureExecutionSucceeded,
		}
		if dailyLimit > 0 {
			var dailyUsed int64
			if err := tx.Model(&aiModel.FrontFeatureUsage{}).
				Where(
		...
		usage = aiModel.FrontFeatureUsage{
			ID: commonModel.NewID(), UserID: userID, Feature: feature,
			CapabilityCode:   capabilityCode,
			RequestID:        utils.RequestIDFromContext(ctx),
			IdempotencyKey:   utils.IdempotencyKeyFromContext(ctx),
			PointCost:        chargedCost,
			ExecutionStatus:  aiModel.FeatureExecutionProcessing,
			BillingStatus:    aiModel.FeatureBillingNotCharged,
		...
		userQuery := tx.Model(&userModel.MiniAppUser{}).
			Where("id = ? AND version = ?", userID, user.Version)
		if chargedCost > 0 {
			usage.BillingStatus = aiModel.FeatureBillingCharged
		...
		if chargedCost > 0 {
			...
			if err := tx.Create(&engagementModel.FrontPointEntry{
				ID: commonModel.NewID(), UserID: userID, Type: engagementModel.PointSpent,
				Scene: scene,
...
func (service *EngagementService) FinishFeatureUsage(
	...
	if success {
		err := service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			...
			if current.ExecutionStatus == aiModel.FeatureExecutionProcessing {
				update := tx.Model(&aiModel.FrontFeatureUsage{}).
					Where("id = ? AND execution_status = ?", current.ID, aiModel.FeatureExecutionProcessing).
					Updates(map[string]interface{}{
						"execution_status": aiModel.FeatureExecutionSucceeded,
				...
				if update.RowsAffected == 1 {
					current.ExecutionStatus = aiModel.FeatureExecutionSucceeded
			...
			if err := tx.Model(&userModel.MiniAppUser{}).Where("id = ?", current.UserID).
	...
	err := service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if current.ExecutionStatus == aiModel.FeatureExecutionProcessing {
			if current.PointCost > 0 &&
				current.BillingStatus == aiModel.FeatureBillingCharged {
				billingStatus = aiModel.FeatureBillingRefundPending
			}
			update := tx.Model(&aiModel.FrontFeatureUsage{}).
				Where("id = ? AND execution_status = ?", current.ID, aiModel.FeatureExecutionProcessing).
				Updates(map[string]interface{}{
					"execution_status": aiModel.FeatureExecutionFailed,
			...
			if update.RowsAffected == 1 {
				current.ExecutionStatus = aiModel.FeatureExecutionFailed
	...
	if billingStatus != aiModel.FeatureBillingRefundPending {
		return featureUsageResult(current, billingStatus, balance), nil
	}
	balance, err = service.refundPendingFeatureUsage(ctx, current.ID)
	if err != nil {
		return featureUsageResult(current, aiModel.FeatureBillingRefundPending, balance),
			appErrors.FrontRefundPending.Wrap(err, "refund feature usage later")
	}
	current.BillingStatus = aiModel.FeatureBillingRefunded
	return featureUsageResult(current, aiModel.FeatureBillingRefunded, balance), nil
}
...
func featureUsageResult(
	...
	case aiModel.FeatureBillingRefundPending:
		value := appErrors.Message(appErrors.FrontRefundPending)
		message = &value
	case aiModel.FeatureBillingRefunded:
...
func (service *EngagementService) refundPendingFeatureUsage(
	...
	err := service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if usage.BillingStatus == aiModel.FeatureBillingRefunded {
			return tx.Model(&userModel.MiniAppUser{}).Where("id = ?", usage.UserID).
				Select("points").Scan(&balance).Error
		}
		if usage.BillingStatus != aiModel.FeatureBillingRefundPending || usage.PointCost <= 0 {
			return appErrors.FrontStateConflict.DefaultMsg()
		}
		claim := tx.Model(&aiModel.FrontFeatureUsage{}).
			Where("id = ? AND billing_status = ?", usage.ID, aiModel.FeatureBillingRefundPending).
			Updates(map[string]interface{}{
				"billing_status": aiModel.FeatureBillingRefunded,
				"updated_at":     now,
			})
		if claim.Error != nil {
		...
		var chargeEntry engagementModel.FrontPointEntry
		if err := tx.Where(
			"user_id = ? AND type = ? AND related_object_type = ? AND related_object_id = ?",
			usage.UserID, engagementModel.PointSpent, "feature_usage", usage.ID,
		...
		objectType := "feature_usage"
		if err := tx.Create(&engagementModel.FrontPointEntry{
			ID: commonModel.NewID(), UserID: usage.UserID, Type: engagementModel.PointRefund,
		...
		if err := tx.Create(&engagementModel.UserNotification{
			ID: commonModel.NewID(), UserID: usage.UserID,
...
func (service *EngagementService) ProcessPendingRefunds(ctx context.Context, batchSize int) error {
	...
	if err := service.database().WithContext(ctx).Model(&aiModel.FrontFeatureUsage{}).
		Where("billing_status = ?", aiModel.FeatureBillingRefundPending).
...
func (service *EngagementService) NotificationSummary(
	...
	db := service.database().WithContext(ctx).Model(&engagementModel.UserNotification{}).
		Where("user_id = ?", userID)
	var total int64
	var unread int64
...
func (service *EngagementService) Notifications(
	...
	statement := service.database().WithContext(ctx).Model(&engagementModel.UserNotification{}).
		Where("user_id = ?", userID)
	...
	var rows []engagementModel.UserNotification
	if err := statement.Order("created_at desc, id desc").
		Limit(query.PageSize).Offset((query.Page - 1) * query.PageSize).Find(&rows).Error; err != nil {
...
func (service *EngagementService) ReadNotification(
	...
	now := service.now()
	result := service.database().WithContext(ctx).Model(&engagementModel.UserNotification{}).
