...
func seedSuggestionGenerationRuntime(
	...
	tomato := dishModel.StandardIngredient{
	...
	egg := dishModel.StandardIngredient{
	...
	dish := dishModel.StandardDishIndex{
	...
	relations := []dishModel.StandardDishIngredient{
		{
	...
	rows := []interface{}{
		...
		&aiModel.AIProvider{
			...
			Type: aiModel.AIProviderOpenAI, BaseURL: endpoint,
			APIKey:    "test-key",
		...
		&aiModel.AICapabilityDefinition{
			Code: aiModel.AICapabilityMealSuggest, Name: "不知道吃什么",
		...
		&aiModel.AICapabilityConfig{
			...
			TimeoutMS:  1000,
			PromptMode: aiModel.AIPromptPreset, PromptPresetVersion: 2,
...
func TestSuggestionCatalogValidationRefundsAfterTwoFailedAttempts(t *testing.T) {
	...
	if err := db.Create(&userModel.MiniAppUser{
		...
		Points: 100, CheckinDayCount: 7, Status: userModel.UserStatusNormal,
	...
	_ = db.Model(&mealModel.FrontMealSuggestion{}).Count(&suggestionCount).Error
