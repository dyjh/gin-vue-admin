...
func dishSummary(dish contentModel.UserDish) frontResponse.DishSummary {
...
func (service *ContentService) loadDishUsageCounts(
	...
	if err := service.database().WithContext(ctx).
		Model(&mealModel.FrontMealFinalDish{}).
		Select("dish_id, COUNT(*) AS usage_count").
		Where("dish_id IN ?", dishIDs).
		Group("dish_id").
...
func dishResponse(dish contentModel.UserDish) frontResponse.Dish {
	...
	for _, ingredient := range dish.Ingredients {
		...
		result.Ingredients = append(result.Ingredients, frontResponse.Ingredient{
			ID: commonModel.NewID(), Name: ingredient.Name, Amount: stringValue(ingredient.Quantity),
	...
	for _, step := range dish.Steps {
		result.Steps = append(result.Steps, frontResponse.DishStep{
			ID: commonModel.NewID(), Text: step.Description, ImageURL: step.ImageURL, SortOrder: step.SortOrder,
		})
	}
	return result
}
...
func officialDishSummary(dish dishModel.OfficialDish) frontResponse.DishSummary {
	tags := make([]string, 0, len(dish.Tags))
	for _, tag := range dish.Tags {
		tags = append(tags, tag.Name)
	}
...
func officialDishResponse(dish dishModel.OfficialDish) frontResponse.Dish {
	result := frontResponse.Dish{
	...
	for _, ingredient := range dish.Ingredients {
		...
		result.Ingredients = append(result.Ingredients, frontResponse.Ingredient{
			ID: commonModel.NewID(), Name: ingredient.Name,
	...
	for _, step := range dish.Steps {
		result.Steps = append(result.Steps, frontResponse.DishStep{
			ID: commonModel.NewID(), Text: step.Description,
...
func (service *ContentService) ListDishes(
	...
	db := service.database()
	statement := db.WithContext(ctx).Model(&contentModel.UserDish{}).
		Where("owner_id = ?", userID)
	if keyword := strings.TrimSpace(query.Q); keyword != "" {
		pattern := "%" + keyword + "%"
		categoryIDs := db.Model(&dishModel.ContentCategory{}).
			Select("id").
			Where("name LIKE ?", pattern)
		ingredientDishIDs := db.Model(&contentModel.DishIngredient{}).
			Select("dish_id").
			Where("name LIKE ?", pattern)
		tagIDs := db.Model(&dishModel.ContentTag{}).
			Select("id").
			Where("name LIKE ?", pattern)
		taggedDishIDs := db.Model(&contentModel.UserDishTag{}).
			Select("dish_id").
	...
	if query.Sort == "frequent" {
		statement = statement.Where(
			"of_dishes.id IN (?)",
			db.Model(&mealModel.FrontMealFinalDish{}).Select("dish_id"),
		)
	}
	var total int64
	...
	var rows []contentModel.UserDish
...
func (service *ContentService) Dish(
	...
	recipeCounts, mealCounts, err := service.loadDishUsageCounts(ctx, []contentModel.UserDish{row})
	if err != nil {
...
func (service *ContentService) resolveCategory(
	tx *gorm.DB,
	value string,
) (dishModel.ContentCategory, error) {
...
func (service *ContentService) resolveAsset(
	...
	fileID string,
	scenes ...string,
) (contentModel.FrontMediaAsset, error) {
...
func (service *ContentService) persistDishChildren(
	...
	if err := tx.Where("dish_id = ?", dish.ID).Delete(&contentModel.DishIngredient{}).Error; err != nil {
		return appErrors.FrontInternal.Wrap(err, "replace dish ingredients")
	}
	if err := tx.Where("dish_id = ?", dish.ID).Delete(&contentModel.DishStep{}).Error; err != nil {
		return appErrors.FrontInternal.Wrap(err, "replace dish steps")
	}
	if err := tx.Where("dish_id = ?", dish.ID).Delete(&contentModel.UserDishTag{}).Error; err != nil {
	...
	for _, item := range input.Ingredients {
		...
		row := contentModel.DishIngredient{
	...
	err := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if dishID == nil {
			row := contentModel.UserDish{
				PublicID: commonModel.NewID(), OwnerID: userID, CoverFileID: cover.ID,
				CoverURL: &cover.URL, Name: strings.TrimSpace(input.Name), CategoryID: category.ID,
				Status: input.Status, Discoverable: false, SourceType: contentModel.SourceTypeManual,
			...
			if preferenceEnabled && input.Status == contentModel.DishStatusUsable {
				if err := ServiceGroupApp.PreferenceService.queueDishEvidenceTx(
					ctx, tx, userID,
					userModel.PreferenceSourceDishSaved, row.PublicID,
		...
		if input.Status != contentModel.DishStatusUsable {
...
func (service *ContentService) DeleteDish(
	...
	var mealCount int64
	err := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&contentModel.RecipeDish{}).Where("dish_id = ?", dish.ID).Count(&recipeCount).Error; err != nil {
			return appErrors.FrontInternal.Wrap(err, "count dish recipe references")
		}
		if err := tx.Model(&mealModel.FrontMealCandidate{}).
			Where("dish_id = ? AND meal_id IN (?)", dish.ID,
				tx.Model(&mealModel.FrontMeal{}).Select("id").
					Where("status IN ?", []mealModel.MealStatus{mealModel.MealCollecting, mealModel.MealClosed}),
			).Count(&mealCount).Error; err != nil {
			return appErrors.FrontInternal.Wrap(err, "count dish meal references")
		}
		if err := tx.Where("dish_id = ?", dish.ID).Delete(&contentModel.RecipeDish{}).Error; err != nil {
...
func recipeResponse(recipe contentModel.UserRecipe) frontResponse.Recipe {
...
func (service *ContentService) Recipes(
	...
	var rows []contentModel.UserRecipe
...
func (service *ContentService) Recipe(
	...
	var row contentModel.UserRecipe
...
func (service *ContentService) replaceRecipeDishes(
	...
	if err := tx.Where("recipe_id = ?", recipe.ID).Delete(&contentModel.RecipeDish{}).Error; err != nil {
		return appErrors.FrontInternal.Wrap(err, "replace recipe dishes")
	}
	for index, publicID := range dishIDs {
		var dish contentModel.UserDish
		if err := tx.First(&dish, "public_id = ? AND owner_id = ? AND status = ?", publicID, userID, contentModel.DishStatusUsable).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return appErrors.FrontBadRequest.DefaultMsg()
			}
		...
		relation := contentModel.RecipeDish{
			PublicID: commonModel.NewID(), RecipeID: recipe.ID, DishID: dish.ID,
...
func (service *ContentService) CreateRecipe(
	...
	row := contentModel.UserRecipe{
		PublicID: commonModel.NewID(), OwnerID: userID, Name: strings.TrimSpace(input.Name),
...
func (service *ContentService) DeleteRecipe(
	...
	return db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("recipe_id = ?", row.ID).Delete(&contentModel.RecipeDish{}).Error; err != nil {
...
func (service *ContentService) Recommendations(
	...
	statement := db.WithContext(ctx).Model(&dishModel.PlatformRecommendation{}).
		Where("selected = ?", true)
	if query.Category != "" {
		categoryIDs := db.Model(&dishModel.ContentCategory{}).Select("id").
			Where("public_id = ? OR name = ?", query.Category, query.Category)
		statement = statement.Where(
			"(source_type = ? AND dish_id IN (?)) OR (source_type = ? AND official_dish_id IN (?))",
			"creator",
			db.Model(&contentModel.UserDish{}).Select("id").Where("category_id IN (?)", categoryIDs),
			"official",
			db.Model(&dishModel.OfficialDish{}).Select("id").Where("category_id IN (?)", categoryIDs),
		)
	}
	var total int64
	if err := statement.Count(&total).Error; err != nil {
...
func (service *ContentService) recommendationSummary(
	...
	var copyCount int64
	if err := service.database().WithContext(ctx).Model(&dishModel.RecommendationCopy{}).
	...
	case "official":
		...
		if err := preloadOfficialDish(service.database().WithContext(ctx)).
			First(&dish, "id = ? AND status = ?", *recommendation.OfficialDishID, contentModel.DishStatusUsable).Error; err != nil {
...
func (service *ContentService) CopyRecommendation(
	...
	preferenceEnabled := preferenceUpdatesEnabled(ctx, userID)
	err := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var existing dishModel.RecommendationCopy
		...
		var sourceIngredients []contentModel.DishIngredient
		var sourceSteps []contentModel.DishStep
		var sourceTags []dishModel.ContentTag
		...
		case "creator":
			...
			copied.SourceType = contentModel.SourceTypeCreatorCopy
		...
		case "official":
			...
			if err := preloadOfficialDish(tx).
				First(&source, "id = ? AND status = ?", *recommendation.OfficialDishID, contentModel.DishStatusUsable).Error; err != nil {
				return appErrors.FrontNotFound.DefaultMsg()
			}
			coverURL := source.CoverURL
			copied = contentModel.UserDish{
				PublicID: commonModel.NewID(), CoverFileID: source.CoverFileID,
				CoverURL: &coverURL, Name: source.Name, CategoryID: source.CategoryID,
				Status: contentModel.DishStatusUsable, SourceType: contentModel.SourceTypeOfficialCopy,
			...
			sourceIngredients = make([]contentModel.DishIngredient, 0, len(source.Ingredients))
			for _, ingredient := range source.Ingredients {
				sourceIngredients = append(sourceIngredients, contentModel.DishIngredient{
			...
			sourceSteps = make([]contentModel.DishStep, 0, len(source.Steps))
			for _, step := range source.Steps {
				sourceSteps = append(sourceSteps, contentModel.DishStep{
		...
		copied.Status = contentModel.DishStatusUsable
		...
		if err := tx.Create(&dishModel.RecommendationCopy{
			ID: commonModel.NewID(), RecommendationID: recommendation.ID,
		...
		if preferenceEnabled {
			if err := ServiceGroupApp.PreferenceService.queueDishEvidenceTx(
				ctx, tx, userID,
				userModel.PreferenceSourceRecommendationAdopted, recommendation.ID,
