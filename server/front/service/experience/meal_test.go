...
func seedMealUser(t *testing.T, db *gorm.DB, id string, now time.Time) {
	...
	if err := db.Create(&userModel.MiniAppUser{
		...
		Nickname: id, Status: userModel.UserStatusNormal,
...
func seedMealDish(
	...
	dish := contentModel.UserDish{
		...
		Name: "测试菜品", CategoryID: category.ID, Status: contentModel.DishStatusUsable,
...
func TestConcurrentMealCreationKeepsSingleActiveCreatorMeal(t *testing.T) {
	...
	if err := db.Model(&mealModel.FrontMeal{}).
		Where("creator_id = ? AND status IN ?", "creator", []mealModel.MealStatus{
...
func TestMealRemainsActiveThroughPurchaseUntilCreatorCompletes(t *testing.T) {
	...
	if err != nil || closed.Status != string(mealModel.MealClosed) {
	...
	var manuallyClosed mealModel.FrontMeal
	if err := db.First(&manuallyClosed, "id = ?", created.ID).Error; err != nil ||
		manuallyClosed.CloseSource == nil ||
		*manuallyClosed.CloseSource != mealModel.MealCloseSourceCreatorAction {
	...
	if err != nil || confirmed.Status != string(mealModel.MealConfirmed) || shoppingListID == "" {
	...
	if err != nil || completed.Status != string(mealModel.MealCompleted) || completed.CompletedAt == nil {
	...
	var storedItem mealModel.FrontShoppingItem
...
func TestDeadlineGuardCommitsClosureBeforeRejectingLateVote(t *testing.T) {
	...
	meal := mealModel.FrontMeal{
		...
		Code: "EXPIRE", Status: mealModel.MealCollecting,
	...
	scannedMeal := mealModel.FrontMeal{
		ID: "deadline-scan-meal", CreatorID: "deadline-creator", Name: "扫描到期饭局",
		Code: "SCAN01", Status: mealModel.MealCollecting,
	...
	var scannedStored mealModel.FrontMeal
	if err := db.First(&scannedStored, "id = ?", scannedMeal.ID).Error; err != nil ||
		scannedStored.Status != mealModel.MealClosed ||
		scannedStored.CloseSource == nil ||
		*scannedStored.CloseSource != mealModel.MealCloseSourceMinuteScan {
...
func TestCreatorRemovesCandidateByCandidateID(t *testing.T) {
	...
	meal := mealModel.FrontMeal{
		...
		Code: "REMOVE", Status: mealModel.MealCollecting,
		DeadlineAt: now.Add(time.Hour), CreatedAt: now, UpdatedAt: now,
	}
	firstCandidate := mealModel.FrontMealCandidate{
		ID: "remove-candidate-1", MealID: meal.ID, DishID: firstDish.ID,
		Available: true, SortOrder: 1, CreatedAt: now,
	}
	secondCandidate := mealModel.FrontMealCandidate{
	...
	var stored mealModel.FrontMealCandidate
...
func TestDeleteDishRetainsInvalidCandidateAndExcludesVotes(t *testing.T) {
	...
	var persisted mealModel.FrontMealCandidate
...
func TestFinalResultSubscriptionIsConsumedByFirstConfirmedResult(t *testing.T) {
	...
	meal := mealModel.FrontMeal{
		ID: "meal-1", CreatorID: "creator", Name: "晚饭", Code: "ABC123",
		Status: mealModel.MealClosed, DeadlineAt: now.Add(time.Hour),
		CreatedAt: now, UpdatedAt: now,
	}
	rows := []interface{}{
		&meal,
		...
		&engagementModel.SubscribeMessageTemplate{
			...
			Scene: engagementModel.SubscribeSceneMealStatus, Purpose: "饭局最终结果",
	...
	recorded, err := service.RecordFinalResultSubscription(
		...
		meal.ID,
		"wechat-template-1",
		engagementModel.MealAuthorizationAccept,
	)
	...
	if err := db.Model(&engagementModel.MealFinalResultSubscription{}).
	...
	var log engagementModel.SubscribeMessageLog
	...
	if subscription.ConsumedAt == nil || subscription.ConsumedResult == nil ||
		*subscription.ConsumedResult != string(mealModel.MealConfirmed) ||
		log.Status != engagementModel.SubscribeLogPending ||
...
func TestRetainedShoppingListIsParticipantReadOnly(t *testing.T) {
	...
	meal := mealModel.FrontMeal{
		...
		Code: "RETAIN", Status: mealModel.MealCancelled,
	...
	if err != nil || !detail.ReadOnly ||
		...
		*detail.CancelledFromStatus != string(mealModel.MealConfirmed) {
	...
	completedRows := []interface{}{
		&mealModel.FrontMeal{
			...
			Code: "DONE01", Status: mealModel.MealCompleted,
