...
func createEngagementTestUser(t *testing.T, db *gorm.DB, now time.Time) userModel.MiniAppUser {
	t.Helper()
...
func seedEngagementCapability(t *testing.T, db *gorm.DB, now time.Time) {
	...
	if err = db.Create(&aiModel.PlatformCapabilityPolicy{
...
func TestCheckinRewardFollowsPlatformSwitchAndCurrentPointRule(t *testing.T) {
	...
	disabledAsset := contentModel.FrontMediaAsset{
		...
		Checksum: "disabled-checksum", ReviewStatus: contentModel.MediaReviewPassed,
		CreatedAt: now,
	}
	...
	var disabledUserAfter userModel.MiniAppUser
	...
	if err := db.Create(&engagementModel.PointRuleConfig{
	...
	var enabledUserAfter userModel.MiniAppUser
	...
	var rewardEntry engagementModel.FrontPointEntry
	if err := db.First(
		&rewardEntry,
		"user_id = ? AND scene = ?",
		enabledUser.ID,
...
func TestCheckinAnalysisDailyLimitUsesCheckinOrder(t *testing.T) {
	...
	for index := 1; index <= 3; index++ {
		asset := contentModel.FrontMediaAsset{
			...
			ReviewStatus: contentModel.MediaReviewPassed, CreatedAt: now,
		}
	...
	if err := db.Model(&userModel.PreferenceEvidence{}).
		Where("user_id = ? AND source_type = ?", user.ID, userModel.PreferenceSourceCheckinImage).
...
func TestFeatureFailureRefundIsIdempotentAndNeverCreatesSubscription(t *testing.T) {
	...
	usage := aiModel.FrontFeatureUsage{
		...
		BillingStatus:    aiModel.FeatureBillingCharged,
	...
	if err := db.Create(&engagementModel.FrontPointEntry{
		ID:                "charge-entry",
		UserID:            user.ID,
		Type:              engagementModel.PointSpent,
		Scene:             "feature_usage",
		Title:             "增强功能使用",
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if err != nil || result.BillingStatus != aiModel.FeatureBillingRefunded ||
	...
	if err := db.Model(&engagementModel.FrontPointEntry{}).
		Where("type = ? AND related_object_id = ?", engagementModel.PointRefund, usage.ID).
