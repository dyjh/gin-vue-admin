...
type capabilityRuntime struct {
	Config            aiModel.AICapabilityConfig
	Model             aiModel.AIModel
	Provider          aiModel.AIProvider
	AuxiliaryModel    *aiModel.AIModel
	AuxiliaryProvider *aiModel.AIProvider
}
...
func (service *AssistService) runImageCapability(
	...
	startedAt := service.now()
	runtime, err := service.capability(ctx, aiModel.AICapabilityDishCoverCreate)
	if err != nil {
		return nil, "", err
	}
	if err := service.recordFeatureUsageRuntime(
...
func (service *AssistService) recordFeatureUsageRuntime(
	...
	if runtime.Config.PromptMode == aiModel.AIPromptPreset {
		promptMode = "default"
	}
	...
	if err := service.database().WithContext(ctx).Model(&aiModel.FrontFeatureUsage{}).
...
func (service *AssistService) ExtractDish(
	...
	if input.ImageFileID != nil {
		...
		if err := service.database().WithContext(ctx).First(&asset,
			...
			*input.ImageFileID, userID, "dish_extract", contentModel.UsableMediaReviewStatuses(),
		).Error; err != nil {
	...
	if draft.Status == "" {
		draft.Status = contentModel.DishStatusDraft
	}
...
type suggestionValidationContext struct {
	Categories          map[string]dishModel.ContentCategory
	Tags                map[string]dishModel.ContentTag
	Units               map[string]dishModel.ContentUnit
	StandardDishes      map[string]dishModel.StandardDishIndex
	StandardIngredients map[string]dishModel.StandardIngredient
}
...
func (service *AssistService) excludedSuggestionNames(
	...
	var rows []mealModel.FrontMealSuggestionDish
	err := service.database().WithContext(ctx).
		Table(mealModel.FrontMealSuggestionDish{}.TableName()+" AS dishes").
		Select("dishes.*").
		Joins(
			"JOIN "+mealModel.FrontMealSuggestion{}.TableName()+
...
func (service *AssistService) recallSuggestionCandidates(
	...
	db := service.database().WithContext(ctx)
	var recommendations []dishModel.PlatformRecommendation
	...
	if err := preloadDish(db).
		Where("status = ? AND category_id IN (?)",
			...
			db.Model(&dishModel.ContentCategory{}).Select("id").Where("enabled = ?", true),
		).
	...
	for _, dish := range dishes {
		...
		if recommendationID != nil && !copied {
			var existingCopy dishModel.RecommendationCopy
			copyErr := db.First(
				&existingCopy,
			...
			if copyErr == nil {
				var copiedDish contentModel.UserDish
	...
	if len(recommendationOfficialDishIDs) > 0 {
		var officialDishes []dishModel.OfficialDish
		if err := preloadOfficialDish(db).
			Where("id IN ? AND status = ?", recommendationOfficialDishIDs, contentModel.DishStatusUsable).
...
func officialDishAsUserDish(dish dishModel.OfficialDish) contentModel.UserDish {
	coverURL := dish.CoverURL
	ingredients := make([]contentModel.DishIngredient, 0, len(dish.Ingredients))
	for _, ingredient := range dish.Ingredients {
		ingredients = append(ingredients, contentModel.DishIngredient{
	...
	steps := make([]contentModel.DishStep, 0, len(dish.Steps))
	for _, step := range dish.Steps {
		steps = append(steps, contentModel.DishStep{
			Description: step.Description, ImageURL: step.ImageURL, SortOrder: step.SortOrder,
		})
	}
	return contentModel.UserDish{
		...
		Status: dish.Status, SourceType: contentModel.SourceTypeOfficialCopy,
		SourceLocked: true, Description: dish.Description, Serving: dish.Serving,
		MediaReviewStatus: contentModel.MediaReviewNotRequired,
...
func (service *AssistService) loadSuggestionValidationContext(
	...
	result := suggestionValidationContext{
		Categories:          make(map[string]dishModel.ContentCategory),
		Tags:                make(map[string]dishModel.ContentTag),
		Units:               make(map[string]dishModel.ContentUnit),
		StandardDishes:      make(map[string]dishModel.StandardDishIndex),
		StandardIngredients: make(map[string]dishModel.StandardIngredient),
	}
	var categories []dishModel.ContentCategory
	var tags []dishModel.ContentTag
...
func (service *AssistService) generateSuggestionDishesWithUsage(
	...
	for attempt := 1; attempt <= attempts; attempt++ {
		...
		runErr := service.runJSONCapability(ctx, aiModel.AICapabilityMealSuggest, map[string]string{
	...
	for _, generated := range output.Dishes {
		...
		for index, generatedStep := range generated.Steps {
			...
			steps = append(steps, frontResponse.DishStep{
				ID: commonModel.NewID(), Text: text, SortOrder: index + 1,
			})
		}
		...
		for index, generatedIngredient := range generated.Ingredients {
			...
			ingredients = append(ingredients, frontResponse.Ingredient{
				ID: commonModel.NewID(), Name: ingredientName, Amount: amount,
...
func (service *AssistService) persistSuggestion(
...
) (mealModel.FrontMealSuggestion, []frontResponse.SuggestionDish, error) {
	if len(dishes) == 0 {
		return mealModel.FrontMealSuggestion{}, nil, appErrors.FrontResultInvalid.DefaultMsg()
	}
	now := service.now()
	...
	if err != nil {
		return mealModel.FrontMealSuggestion{}, nil,
			appErrors.FrontInternal.Wrap(err, "encode suggestion dish ids")
	}
	row := mealModel.FrontMealSuggestion{
		ID: commonModel.NewID(), UserID: userID, Source: source, SourceLabel: sourceLabel,
	...
	err = service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		for index, snapshot := range dishes {
			...
			snapshotRow := mealModel.FrontMealSuggestionDish{
	...
	if err != nil {
		return mealModel.FrontMealSuggestion{}, nil, err
	}
	...
	if finishErr != nil {
		...
		_ = service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			if err := tx.Where("suggestion_id = ?", row.ID).
				Delete(&mealModel.FrontMealSuggestionDish{}).Error; err != nil {
				return err
			}
	...
	if input.Preference != nil && strings.TrimSpace(*input.Preference) != "" {
		...
		_ = service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			return ServiceGroupApp.PreferenceService.queueEvidenceTx(
				ctx, tx, userID,
				userModel.PreferenceSourceExplicitSetting,
...
func (service *AssistService) Suggestion(
	...
	var row mealModel.FrontMealSuggestion
	...
	var snapshotRows []mealModel.FrontMealSuggestionDish
	...
	for _, snapshotRow := range snapshotRows {
		...
		if snapshotRow.CopiedDishID != nil {
			var copied contentModel.UserDish
...
func (service *AssistService) CopySuggestion(
	...
	err := service.database().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var suggestion mealModel.FrontMealSuggestion
		...
		var snapshotRow mealModel.FrontMealSuggestionDish
		if err := tx.
		...
		if snapshotRow.Source == "library" && snapshotRow.SourceDishID != nil {
			...
			if err := preloadOfficialDish(tx).
				First(&source, "id = ? AND status = ?", *snapshotRow.SourceOfficialDishID, contentModel.DishStatusUsable).Error; err != nil {
				return appErrors.FrontNotFound.DefaultMsg()
			}
			var err error
		...
		if snapshotRow.RecommendationID != nil {
			copyRecord := dishModel.RecommendationCopy{
				ID: commonModel.NewID(), RecommendationID: *snapshotRow.RecommendationID,
		...
		if preferenceEnabled {
			if err := ServiceGroupApp.PreferenceService.queueDishEvidenceTx(
				ctx, tx, userID,
				userModel.PreferenceSourceRecommendationAdopted, snapshotRow.ID,
...
func (service *AssistService) copySuggestionOfficialDish(
	tx *gorm.DB,
	userID string,
	source dishModel.OfficialDish,
	snapshotRow mealModel.FrontMealSuggestionDish,
) (contentModel.UserDish, error) {
	coverURL := source.CoverURL
	operationSourceType := "meal_suggestion"
	copied := contentModel.UserDish{
		PublicID: commonModel.NewID(), OwnerID: userID,
		CoverFileID: source.CoverFileID, CoverURL: &coverURL,
		Name: source.Name, CategoryID: source.CategoryID,
		Status: contentModel.DishStatusUsable, Discoverable: false,
		SourceType: contentModel.SourceTypeMealSuggestionCopy, SourceLocked: true,
	...
	for _, ingredient := range source.Ingredients {
		if err := tx.Create(&contentModel.DishIngredient{
	...
	for _, step := range source.Steps {
		if err := tx.Create(&contentModel.DishStep{
	...
	for _, tag := range source.Tags {
		if err := tx.Create(&contentModel.UserDishTag{
...
func (service *AssistService) copySuggestionLibraryDish(
	...
	copied.PublicID = commonModel.NewID()
	copied.OwnerID = userID
	copied.Owner = userModel.MiniAppUser{}
	copied.Discoverable = false
	copied.DiscoverableAt = nil
	copied.SourceLocked = true
	copied.SourceType = contentModel.SourceTypeMealSuggestionCopy
	operationSourceType := "meal_suggestion"
...
func (service *AssistService) copyGeneratedSuggestionDish(
	...
	if err := tx.First(
		...
		return contentModel.UserDish{}, appErrors.FrontStateConflict.Wrap(
	...
	operationSourceType := "meal_suggestion"
	row := contentModel.UserDish{
		PublicID: commonModel.NewID(), OwnerID: userID,
		CoverFileID: "", CoverURL: nil, Name: strings.TrimSpace(snapshot.Name),
		CategoryID: category.ID, Status: contentModel.DishStatusDraft,
		Discoverable: false, SourceType: contentModel.SourceTypeGeneratedSuggestion,
	...
	for _, snapshotIngredient := range snapshot.Ingredients {
		var unit dishModel.ContentUnit
		if err := tx.First(
			&unit,
			"name = ? AND enabled = ?",
...
func (service *AssistService) Feedback(
	...
	var model interface{}
	if kind == "suggestion" {
		model = &mealModel.FrontMealSuggestion{}
	} else {
		model = &mealModel.FrontPrepPlan{}
	}
...
func prepSourcesFromFinals(
	finals []mealModel.FrontMealFinalDish,
...
func (service *AssistService) excludedPrepPlans(
	...
	var rows []mealModel.FrontPrepPlan
	if err := service.database().WithContext(ctx).
	...
	rowByID := make(map[string]mealModel.FrontPrepPlan, len(rows))
	for _, row := range rows {
		rowByID[row.ID] = row
	}
...
func (service *AssistService) failPrepUsage(
	ctx context.Context,
	usage aiModel.FrontFeatureUsage,
	cause error,
) error {
...
func (service *AssistService) GeneratePrepPlan(
	...
	if meal.Status != mealModel.MealConfirmed {
	...
	usage, _, err := service.engagement().BeginFeatureUsage(
		ctx, userID, aiModel.AICapabilityPrepSequence, true,
	)
	if err != nil {
		return frontResponse.PrepPlan{}, err
	}
	...
	now := service.now()
	row := mealModel.FrontPrepPlan{
		ID: commonModel.NewID(), UserID: userID, MealID: input.MealID,
...
func (service *AssistService) PrepPlan(
	...
	var row mealModel.FrontPrepPlan
