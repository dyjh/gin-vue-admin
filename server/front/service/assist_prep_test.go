...
func seedPrepRuntime(t *testing.T, db *gorm.DB, endpoint string) {
	...
	rows := []interface{}{
		...
		&orderfoodModel.AICapabilityDefinition{
			Code: orderfoodModel.AICapabilityPrepSequence, Name: "饭局备菜顺序",
			ClientFeatureCode: "prep_sequence", RequiredModelCapability: orderfoodModel.AIModelText,
			SortOrder: 1, CreatedAt: now, UpdatedAt: now,
		},
		&orderfoodModel.AICapabilityConfig{
			CapabilityCode: orderfoodModel.AICapabilityPrepSequence,
			Version:        1, PrimaryModelID: "model-1", PointCost: 7,
			TimeoutMS:  1000,
			PromptMode: orderfoodModel.AIPromptPreset, PromptPresetVersion: 1,
...
func seedConfirmedPrepMeal(t *testing.T, db *gorm.DB) {
	...
	rows := []interface{}{
		&orderfoodModel.MiniAppUser{
			...
			Nickname: "测试用户", Points: 100, Status: orderfoodModel.UserStatusNormal,
			RegisteredAt: now, CreatedAt: now, UpdatedAt: now,
		},
		&orderfoodModel.FrontMeal{
			ID: "meal-1", CreatorID: "user-1", Name: "晚饭", Code: "ABC123",
			Status: orderfoodModel.MealConfirmed, DeadlineAt: now.Add(time.Hour),
...
func TestGeneratePrepPlanUsesCurrentModelAndPersistsValidatedSnapshotSteps(t *testing.T) {
	...
	_ = db.Model(&orderfoodModel.FrontPrepPlan{}).Count(&planCount).Error
