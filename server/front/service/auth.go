...
type LoginResult struct {
	...
	User        orderfoodModel.MiniAppUser // 当前用户
}
...
func (service *AuthService) Login(
	...
	transactionErr := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		codeRecord := orderfoodModel.MiniAppLoginCode{
		...
		if err := tx.Model(&orderfoodModel.MiniAppUser{}).
			Where("id = ?", user.ID).
		...
		loginSession := orderfoodModel.MiniAppSession{
			ID:        orderfoodModel.NewID(),
...
func (service *AuthService) Authenticate(
	...
	if db == nil || strings.TrimSpace(accessToken) == "" {
		...
		return orderfoodModel.MiniAppUser{}, appErrors.FrontLoginExpired.DefaultMsg()
	}
	var loginSession orderfoodModel.MiniAppSession
	if err := db.WithContext(ctx).
		First(&loginSession, "token_hash = ?", digest(accessToken)).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return orderfoodModel.MiniAppUser{}, appErrors.FrontLoginExpired.DefaultMsg()
		}
		return orderfoodModel.MiniAppUser{}, appErrors.FrontInternal.Wrap(err, "load miniapp session")
	}
	now := service.now()
	...
	if err := db.WithContext(ctx).First(&user, "id = ?", loginSession.UserID).Error; err != nil {
		...
		return orderfoodModel.MiniAppUser{}, appErrors.FrontInternal.Wrap(err, "load miniapp session user")
	}
	if user.Status == orderfoodModel.UserStatusDisabled {
		return orderfoodModel.MiniAppUser{}, appErrors.FrontUserDisabled.DefaultMsg()
	}
	if err := db.WithContext(ctx).Model(&orderfoodModel.MiniAppSession{}).
		Where("id = ?", loginSession.ID).
		Update("last_used_at", now).Error; err != nil {
		return orderfoodModel.MiniAppUser{}, appErrors.FrontInternal.Wrap(err, "touch miniapp session")
	}
	return user, nil
}
