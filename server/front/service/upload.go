...
var allowedUploadScenes = map[string]orderfoodModel.ModerationScene{
	"profile_avatar": orderfoodModel.ModerationSceneProfileAvatar,
	"dish_cover":     orderfoodModel.ModerationSceneDishCover,
	"dish_step":      orderfoodModel.ModerationSceneDishStep,
	"dish_extract":   orderfoodModel.ModerationSceneDishExtract,
	"checkin":        orderfoodModel.ModerationSceneCheckin,
}

// UploadService 提供小程序图片上传业务能力。
type UploadService struct {
	DB         *gorm.DB                             // 业务数据库
	Moderation orderfoodService.ImageModerationGate // 用户图片强制审核门禁
...
func (service *UploadService) moderation() orderfoodService.ImageModerationGate {
	if service != nil && service.Moderation != nil {
		return service.Moderation
	}
	return orderfoodService.ServiceGroupApp.Moderation
}
...
func (service *UploadService) persistImage(
	...
	userID string,
	scene string,
	moderationScene orderfoodModel.ModerationScene,
	contentType string,
	payload []byte,
	requestID string,
) (frontResponse.ImageUploadResult, error) {
