...
Page({
  ...
  async onLoad() {
    try {
      ...
      this.setData({ suggestionFeature });
  ...
  async runGenerate(useFreeQuota) {
    ...
    try {
      const result = await api.generateWhatToEat({
        ...
        excludedSuggestionDishIds: this.data.excludedSuggestionDishIds,
        useFreeQuota,
      });
      ...
      const status = {
        ...
        pointBalance: Number(result.usage?.pointBalance ?? this.data.status.pointBalance),
      };
      ...
      this.setData({
        ...
        resultReason: result.reason || ("你选择了" + selectedTasteText + "，已为你组合适合今天的菜品。"),
        resultReasonExtra: "这组搭配已按 " + this.data.people + " 人调整菜品数量。",
        selectedTasteText,
        viewMode: "result",
        showPointsConfirm: false,
      });
  ...
  openDetail(event) {
    ...
    go("/pages/ideas/what-to-eat-detail", {
  ...
  async regenerate() {
    if (!this.data.result) return;
    await api.submitMealSuggestionFeedback(this.data.result.id, "regenerated");
    const suggestionDishIds = this.data.resultDishes
      .map((dish) => dish.id)
      .filter(Boolean);
    const excludedSuggestionDishIds = Array.from(new Set([
      ...this.data.excludedSuggestionDishIds,
      ...suggestionDishIds,
    ]));
    this.setData({ excludedSuggestionDishIds });
    this.generate();
  },

  async ignore() {
    if (!this.data.result) return;
    await api.submitMealSuggestionFeedback(this.data.result.id, "ignored");
    this.setData({
      excludedSuggestionDishIds: Array.from(new Set([
        ...this.data.excludedSuggestionDishIds,
        ...this.data.resultDishes.map((dish) => dish.id).filter(Boolean),
      ])),
      result: null,
      viewMode: "conditions",
    });
