...
- `../prd/order-food-admin-menu-page-review.md`：6 个菜单分组、2 个一级直达页面、28 个页面的菜单顺序、页面按钮和角色可见性实施基线。
...
- 所有写接口声明按钮权限和幂等策略。
- 所有页面引用的接口都真实存在。
- 页面固定为 6 个菜单分组、2 个一级直达页面、28 个叶子页面，页码、路由名和排序唯一。
- 所有接口使用 `/api/orderfood` 管理员认证边界。
- 28 个页面组件、114 条 Swagger 接口和 114 个 Web API 封装与契约完全一致。
- 87 个业务权限均进入 GVA 按钮权限真源，且没有契约外权限。

当前生成统计：28 个页面、114 条接口、141 个模型、87 个权限码、4 类默认角色模板。
